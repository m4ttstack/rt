 ...
 * augmentation below, which references `ExtendedCustomColors` below -- that
 * type derives its keys from this object, so the two can't drift out of
 * sync.
 */
 ...
 * augmentation below, so that ambient declaration and this union can't
 * drift apart.
 */
...
/**
 * Lives here rather than in a standalone `.d.ts`: an ambient module file is
 * only seen by consumers that reference it, but this file is imported by
 * `theme.ts` (in turn imported by `mount.tsx` and `test-utils`), so the
 * augmentation rides along with any real use of the kit.
 */
