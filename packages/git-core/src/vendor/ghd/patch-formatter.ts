...
import type { PatchTarget, TextDiffLike } from './types'
import {
  DiffLineType,
} from './diff-line'
import {
 ...
 * on the file state of the given PatchTarget
 */
function formatPatchHeaderForFile(file: PatchTarget) {
  switch (file.status.kind) {
    ...
    // I'd like to get to a point where `PatchTarget` can be
    ...
    default:
      return assertNever(file.status.kind, `Unknown file status ${file.status.kind}`)
  }
}
