...
#
# console is a public, general-purpose developer tool. Nothing about any
...
# as source.
#
# The bar exists because it was crossed in a sibling repo (rt): it was public
# for six months carrying an internal GitLab host, real ticket ids and titles,
# internal repo names, and two named customers wired to database resource
# names in test fixtures. A word list cannot certify what it was never told to
# look for, so add to it whenever a new term shows up rather than assuming
...
# patterns (observed: design/wiring/reference/*.png), and nothing is authored
# in them either.
HITS=$(cd "$ROOT" \
  && git ls-files -z \
...
if [ -n "$HITS" ]; then
  ...
  printf '%s\n' "$HITS"
  echo ""
  echo "console is public. Use neutral placeholders (acme, DEMO-1234, gitlab.example.com)."
  exit 1
fi
echo "ok   repo-purity"
