...
10. **Typed skill invocation.** Under its own brainstorm (2026-09-02, in
    flight): a `skillPrompt` serializer in rt-client with skill-arg types
    generated by the skills compiler. This spec's board launch lane consumes
    whatever that design lands; until it lands, the launch lane uses board's
    existing `dispatchPrompt` builder unchanged.
