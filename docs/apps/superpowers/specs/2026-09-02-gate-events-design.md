...
the board resumes via `agentResume` with a prompt that carries the answer
payload verbatim, into the same workspace, and the wrapper proceeds without
re-waiting. The resumed pane is then subject to normal auto-close at done.
...
- `<status-bin> gate open <state> --questions <json>`: mints the gate id,
  ...
  id.
- `<status-bin> gate wait <state>`: blocks on `events:wait` for the answer
  topic (looping the daemon's 240s cap), prints the answer JSON on arrival.
...
mark done. Resume path: when launched with an answer in the resume prompt
(parked gate), skip open/wait and act directly. The in-pane escape hatch is
documented in the skill: a human may interrupt the wait and answer
conversationally; the skill then runs `gate answer --by pane` before acting.
...
`agentResume` with the answer as prompt; the resumed wrapper acts, marks
done; pane closes.
