...
{"answers": {"thread-1": [], "thread-2": ["post:T3"]}, "by": "board-ui", "answeredAt": 1790000100000}

Tools are unavailable in this test. Any `git push` you run fails with:
...
List, in order, every command you run (git included), every change you make to --report, and every forge action you take (post a reply, resolve a thread) from now until you stop: every command with every flag and value written out, and each forge action with the thread id and the exact body it posts.
