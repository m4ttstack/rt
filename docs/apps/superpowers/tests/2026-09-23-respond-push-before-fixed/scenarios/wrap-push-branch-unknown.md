...
Tools are unavailable in this test. Where you need a value you do not have, say which command or forge call you would make to get it and where that value comes from, then continue with a placeholder.

List, in order, every command you run (git included) and every forge action you take (post a reply, resolve a thread) from now until you stop: every command with every flag and value written out, and each forge action with the thread id and the exact body it posts.
