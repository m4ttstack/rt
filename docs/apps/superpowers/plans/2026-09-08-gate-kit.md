...
`packages/gate-kit/test/payload.test.ts` -- port from the same board file: the seven `gateAnswerPayload` tests (lines 35-109) and the four `unwrapGateAnswer` tests (lines 111-134), with these deltas, everything else verbatim:
...
`packages/gate-kit/src/options.ts` (the board copy is the superset: console's `displayValueLabel(value, options)` is `displayForValue(value, options).text`):
...
`packages/gate-kit/src/kinds.ts` -- move the registry from `apps/board/src/gates/sweep.ts` lines 6-49 (the `GateDomain` type, `GATE_KINDS`, `domainForKind`; NOT `SweepAction`/`GateSweepStates`, which stay board-owned):
...
`packages/gate-kit/src/server/index.ts` -- `normalizeWorktreePath` and `panesForOrigin` (with their doc comments and the `PanesForOriginResult` interface) copy verbatim from `apps/board/src/gates/focus.ts` lines 19-29 and 54-85; `resolveOriginFocus` merges the board and console variants behind opts:
...
describe('questionnaire round trip', () => {
  test('the code-changes item appears in the DOM when a fix is picked, and disappears again', async () => {
...
/** Mantine-styled layer over the kit's questionnaire adapter. Every visible
    ...
    multi-step triage modal), and the native form is the one door to
    submission: FormData, not component state, builds the answer. The
    `selections` mirror exists only to drive the structural collapse and the
    submit-disabled derivation in GateCard. */
...
- The collapse test at line 323 ("hides code-changes until a fix is picked and submits the sentinel while hidden") is now structural but observable behavior is identical (the question's inputs are absent from the DOM until a `fix:` pick, and the posted body carries the sentinel); the existing queries keep passing -- verify, do not rewrite.
...
- Modify: `apps/board/src/gates/sweep.ts` (lines 6-49: re-export the registry from the kit)
...
In `apps/board/package.json` dependencies, alphabetically after `"@mattstack/glance"`:
...
In `apps/board/src/gates/sweep.ts`, replace lines 6-49 (the `GateDomain` type through the end of `domainForKind`) with:
...
placed with the other imports at the top of the file; the rest of sweep.ts (`SweepAction`, `GateSweepStates`, `tabIdFor`, `planSweep`, `pruneOffBoardGates`) keeps using `domainForKind`/`GateDomain` unchanged.
...
In `apps/board/src/__tests__/gates-cache.test.ts`, in the `describe('attachGates', ...)` block (line 384): every expected gate object gains `subject: 'mr:<the webUrl used by that test>'`, and each expected ANSWERED gate object (the ones with `answers: { q1: 'yes' }`) additionally gains `answeredBy: 'board-ui'` and `answeredAt: 2000` (matching the fixtures' `answer: { answers: { q1: 'yes' }, by: 'board-ui', answeredAt: 2000 }`). Open/parked expectations gain only `subject`.

Run: `cd apps/board && bun test src/__tests__/gates-cache.test.ts`
Expected: FAIL -- the projection does not carry the new fields yet.
...
If `.tui-gate-thread-group` / `.tui-gate-thread-heading` / `.tui-gate-summary-note` rules already exist further down the file (they predate this change: the old GateCard used all three), keep the existing rules and add only what is missing -- check with `grep -n "tui-gate-thread\|tui-gate-summary-note" apps/board/src/style.css` first.
...
Expected: all four empty -- no stragglers importing the deleted modules, no app importing `@shadcn/react` around the kit's one door, no em/en dashes.
