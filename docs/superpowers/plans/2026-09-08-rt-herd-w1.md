...
describe("rt herd end to end", () => {
  ...
  test("worker ask pushes to the shepherd; shepherd answer nudges the worker", async () => {
    ...
    const spawned = await finished(runRt(["herd", "spawn", "--herd", herd.herd, "--job", "acme-1", "--brief", brief, "--dir", repo, "--json"], home));
    ...
    const asked = await finished(runRt(["herd", "ask", "--questions", JSON.stringify([{ id: "q", label: "Which?", multi: false, options: ["a", "b"] }]), "--json"], home,
      { HERD_ID: herd.herd, HERD_JOB: "acme-1", CLAUDE_CODE_SESSION_ID: "sess-worker" }));
    ...
    expect(JSON.parse(status.stdout).jobs[0]).toMatchObject({ name: "acme-1", status: "active" });
  });
});
```
