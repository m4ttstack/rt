...
describe("herd-store", () => {
  ...
  test("upsertJob inserts then updates the same (herd, name) row", () => {
    const s = store();
    const h = herd(s);
    const j = s.upsertJob({ herd: h.id, name: "acme-1", worktree: "/w/acme-1", handle: "acme-1", status: "spawning" });
    expect(j).toMatchObject({ herd: h.id, name: "acme-1", status: "spawning", pane: null });
    const j2 = s.upsertJob({ herd: h.id, name: "acme-1", worktree: "/w/acme-1", handle: "acme-1", status: "spawning", pane: "w1:p1", agentSession: "sess-w" });
...
describe("herd:spawn", () => {
  ...
  test("provisions, starts the agent in the herd workspace with env and handle, signs the pane in, records the job", async () => {
    ...
    expect(agentCalls[0]).toMatchObject({
      ...
      workspace: `herd: ${herd}`, tab: "acme-1", label: "acme-1", caller: `herd:${herd}`,
    ...
    expect(store.getJob(herd, "acme-1")).toMatchObject({ worktree: "/w/acme-1", branch: "acme-1", pane: "w9:p1", agentSession: "sess-w1", agentId: "ag-1", handle: "acme-1", status: "spawning" });
    expect(res.data).toMatchObject({ pane: "w9:p1", worktree: "/w/acme-1", sessionId: "sess-w1" });
...
describe("herd:wrap-up", () => {
  async function withTwoJobs() {
    ...
    if (!s.ok) throw new Error(s.error);
    const herd = s.data.herd;
    hx.store.upsertJob({ herd, name: "acme-1", worktree: "/w/acme-1", branch: "acme-1", handle: "acme-1", status: "done", pane: "w9:p1" });
    hx.store.upsertJob({ herd, name: "acme-2", worktree: "/w/acme-2", branch: "acme-2", handle: "acme-2", status: "done", pane: "w9:p2" });
  ...
  test("runs exactly the flagged actions: panes, workspace, dispose, job dirs, archive", async () => {
    ...
    if (!res.ok) throw new Error(res.error);
    expect(res.data).toEqual({ closed: ["acme-1", "acme-2"], workspaceClosed: true, disposed: ["acme-1"], refused: [], deletedJobDirs: true, archived: true });
    expect(herdrCalls).toContainEqual(["pane", "close", "w9:p1"]);
    expect(herdrCalls).toContainEqual(["pane", "close", "w9:p2"]);
    expect(herdrCalls.some((c) => c[0] === "workspace" && c[1] === "close")).toBe(true);
    expect(worktreeCalls).toContainEqual({ verb: "dispose", p: { repoName: "gh:m4ttstack/rt", tree: "acme-1" } });
  ...
  test("with no flags it only marks the herd wrapped and reports a dispose refusal verbatim", async () => {
    ...
    if (!s.ok) throw new Error(s.error);
    refusing.store.upsertJob({ herd: s.data.herd, name: "acme-1", worktree: "/w/acme-1", branch: "acme-1", handle: "acme-1", status: "done" });
    const res = await refusing.h["herd:wrap-up"]({ herd: s.data.herd, dispose: ["acme-1"] });
    if (!res.ok) throw new Error(res.error);
    expect(res.data).toMatchObject({ closed: [], workspaceClosed: false, disposed: [], refused: [{ tree: "acme-1", reason: "unmerged work" }], deletedJobDirs: false, archived: false });
