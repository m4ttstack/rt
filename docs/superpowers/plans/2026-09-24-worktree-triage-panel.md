...
10. Interaction states: secondary and primary buttons in rest, hover, pressed, disabled and busy each read as distinct from their neighbours the way the board's do (hover a step lighter/darker, pressed a further step, disabled faded, busy with spinner and in-progress label); the row hover lifts the surface and darkens the border without shifting layout; the busy row dims text and `…` but not the spinner button; link chips underline with the arrow glyph on hover and the push chip doesn't; the bulk button's busy label counts.
