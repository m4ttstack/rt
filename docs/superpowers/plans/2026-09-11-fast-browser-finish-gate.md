...
4. **Overturned in review.** The first cut detected "waived" by the row's note prefix (`Skipped on this Mac`); that coupling was unpinned on the TS side. The contract now carries two distinct booleans on `Row`, both optional and default false: `finishGated` (the row blocks Finish until ready, skipped, or waived) and `waived` (the user skipped it on this Mac; set by `applyFinishGate` from `setup.waived`). Clients read `waived` for the skipped state and the Un-skip affordance and never parse `optionalNote`; the note is copy. The Task 5 interfaces below predate this ruling: `FinishGate.waivedNotePrefix` no longer exists and `PlanRow.isWaived` reads `finishGated && waived`.
