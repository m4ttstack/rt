...
**Fix round 1 (post-review) strengthened the approval checks above and rewrote the merge
cycle below.** The original Step 1/Step 2 code as first written passed spec review but a
live run surfaced three problems: an unmet GitLab fixture precondition (pipeline-must-
succeed) was reported as a hard `fail` indistinguishable from a real provider defect, with
its two downstream assertions silently absent from the report rather than explicitly
skipped; the approve/unapprove checks only asserted "did not throw" and would pass for a
silent no-op; and an unguarded setup step plus an unguarded cleanup read could each crash
the whole run or leave a branch behind. The blocks below reflect the fixed code that
actually ran; see `task-6-report.md` for the full before/after evidence.
...
The step title always meant both `runWriteConformance` and `runMergeConformance`, per this
task's own Interfaces section and Step 4's stated expectation ("the write and merge cycles
run on both providers"); the import below adds both, not just the write cycle.

In `packages/glance/tests/live/runner.ts`, change the import:
...
and wrap the fixture loop's body in a per-fixture `try`/`catch`, calling both new functions
after `runUnsupportedConformance`:
...
(Fix round 1: the per-fixture `try`/`catch` was added after a live run confirmed an
unguarded crash in one fixture's cycle would otherwise erase the other fixture's already-
gathered results. The original step only showed the write-cycle import/call; see
`task-6-report.md`.)
