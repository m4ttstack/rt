/**
 * How `rt chat sign-in` gives the Claude Code session the same name as the
 * chat handle it was just assigned, so the pane, the `--resume` picker and
 * the buddy list all say `fred`.
 *
 ...
 * <handle>` and submits when the agent's turn ends, which is what updates
...
export function planSessionRename(args: {
  ...
  if (pane) return { via: "herdr", argv: ["herdr", "pane", "run", pane, `/rename ${args.handle}`] };
  return { via: "claude", argv: ["claude", "-p", "--resume", args.sessionId, `/rename ${args.handle}`] };
}
