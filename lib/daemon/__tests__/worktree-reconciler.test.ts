...
describe("replenish / shrink", () => {
  ...
  test("S077: a team onDeck above the ceiling is clamped to WORKTREE_ONDECK_CEILING", async () => {
    ...
    await __test__.replenishAndShrink(
      { repoName, repoPath: repo, emit: () => {}, log: fakeLog(), findRunningRun: () => null },
      new Map(),
      fakeAppConfig(),
    );
  ...
  test("S077: free disk below the threshold on cfg.root skips create and warns, without charging a create failure", async () => {
    ...
    try {
      await __test__.replenishAndShrink(
        { repoName, repoPath: repo, emit: () => {}, log, findRunningRun: () => null },
        new Map(),
        fakeAppConfig(),
      );
    } finally {
  ...
  test("a failed create backs off, and the next pass skips replenish for that repo", async () => {
    ...
    const deps = { repoName, repoPath: repo, emit: () => {}, log, findRunningRun: () => null };
