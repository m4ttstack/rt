...
test("agent:start herdr reserves a handle not held by live presence, passes it as --name, and merges crossSessionInbound with the gate-fork hook into one --settings file", async () => {
  ...
  // pins only applies when the launch carries one (acme7 arming
...
// subject is required for the hook half of that merge (acme7 arming
...
// acme7 arming ruling: a launch with no explicit subject gets no gate-
