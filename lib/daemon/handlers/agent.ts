 ...
 * carries an explicit subject (acme7 arming ruling: a launch with no
 ...
 * carries no explicit subject (acme7 arming ruling -- with no
...
export function createAgentHandlers(opts: {
  ...
  return {
    "agent:start": async (rawPayload: unknown): Promise<CommandResult<"agent:start">> => {
      ...
      // caller passes no subject (acme7 arming ruling): launch()'s
