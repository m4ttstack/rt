...
function unlinkLegacyReactorState(): void {
  try {
    unlinkSync(reactorStatePath());
  } catch {
...
function loadReactorState(): ReactorState {
  const raw = getKvValue<Partial<ReactorState>>(REACTOR_STATE_NS, REACTOR_STATE_KEY, {});
  return {
    mrState: raw?.mrState ?? {},
