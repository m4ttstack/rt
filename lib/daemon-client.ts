...
import type { EvidenceSlot } from "./sandbox.ts";

const EVIDENCE_TIMEOUT_MS = 30_000;
const EVIDENCE_PULL_TIMEOUT_MS = 120_000;   // pull moves artifact bytes

/** Request a new evidence capture. `executor` and `sandboxId` are optional -- the daemon decides. */
export async function evidenceRequest(payload: {
  repoId: string;
  branch: string;
  sandboxId?: string;
  caseId: string;
  ...
  forceBefore?: boolean;
}): Promise<{ requestId: string; executor: string }> {
  const res = await daemonQuery("evidence:request", payload, EVIDENCE_TIMEOUT_MS);
  if (!res) throw new Error(lastQueryTimedOut() ? "evidence request timed out" : "daemon unavailable");
  if (!res.ok) throw new Error(res.error || "evidence:request failed");
  return res.data as { requestId: string; executor: string };
}

/** List ledger entries, optionally filtered by branch, sandboxId, and/or state. */
export async function evidenceList(
  filter?: { branch?: string; sandboxId?: string; states?: string[] },
): Promise<EvidenceLedgerEntry[]> {
  const res = await daemonQuery("evidence:list", filter, EVIDENCE_TIMEOUT_MS);
  if (!res) throw new Error(lastQueryTimedOut() ? "evidence list timed out" : "daemon unavailable");
  if (!res.ok) throw new Error(res.error || "evidence:list failed");
  return res.data as EvidenceLedgerEntry[];
}

/** Pull one captured request's artifacts into the evidence tree, or every captured entry when omitted. */
export async function evidencePull(requestId?: string): Promise<{ synced: string[] }> {
  const res = await daemonQuery("evidence:pull", { requestId }, EVIDENCE_PULL_TIMEOUT_MS);
  if (!res) throw new Error(lastQueryTimedOut() ? "evidence pull timed out" : "daemon unavailable");
  if (!res.ok) throw new Error(res.error || "evidence:pull failed");
  return res.data as { synced: string[] };
}

/** Approve or reject a synced request. `reason` is required for a reject. */
export async function evidenceDecide(
  requestId: string,
  decision: "approved" | "rejected",
  reason?: string,
): Promise<void> {
  const cmd = decision === "approved" ? "evidence:approve" : "evidence:reject";
  const res = await daemonQuery(cmd, { requestId, reason }, EVIDENCE_TIMEOUT_MS);
  if (!res) throw new Error(lastQueryTimedOut() ? "evidence decide timed out" : "daemon unavailable");
  if (!res.ok) throw new Error(res.error || `${cmd} failed`);
}

/** File a local-chrome capture's screenshot(s) into the evidence tree, flipping it to synced. */
export async function evidenceFulfill(
  requestId: string,
  basePath: string,
  annotatedPath?: string,
): Promise<void> {
  const res = await daemonQuery("evidence:fulfill", { requestId, basePath, annotatedPath }, EVIDENCE_TIMEOUT_MS);
  if (!res) throw new Error(lastQueryTimedOut() ? "evidence fulfill timed out" : "daemon unavailable");
  if (!res.ok) throw new Error(res.error || "evidence:fulfill failed");
}

/** Replace a synced/approved request's annotated file with a freshly-marked-up one. */
export async function evidenceRedraw(requestId: string, annotatedPath: string): Promise<void> {
  const res = await daemonQuery("evidence:redraw", { requestId, annotatedPath }, EVIDENCE_TIMEOUT_MS);
  if (!res) throw new Error(lastQueryTimedOut() ? "evidence redraw timed out" : "daemon unavailable");
  if (!res.ok) throw new Error(res.error || "evidence:redraw failed");
}

/** Mark an approved request attached (to the MR the evidence was captured for). */
export async function evidenceMarkAttached(requestId: string): Promise<void> {
  const res = await daemonQuery("evidence:mark-attached", { requestId }, EVIDENCE_TIMEOUT_MS);
  if (!res) throw new Error(lastQueryTimedOut() ? "evidence mark-attached timed out" : "daemon unavailable");
  if (!res.ok) throw new Error(res.error || "evidence:mark-attached failed");
}
