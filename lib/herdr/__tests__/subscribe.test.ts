...
describe("subscribeHerdrEvents", () => {
  ...
  test("a partial line across chunks is reassembled", async () => {
    ...
    cleanups.push(() => sub.stop());
    await until(() => sub.connected());
    const line = JSON.stringify({ event: "pane.agent_status_changed", data: { pane_id: "w1:p1", agent: "spike-bot", agent_status: "blocked", workspace_id: "w1" } }) + "\n";
    fake.writeRaw(line.slice(0, 10));
    await Bun.sleep(5);
    fake.writeRaw(line.slice(10));
    await until(() => events.length === 1);
    expect(events[0]).toMatchObject({ type: "pane.agent_status_changed", pane_id: "w1:p1", agent_status: "blocked" });
  });
});
