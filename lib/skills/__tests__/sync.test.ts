...
function makeDeps(pack: PackInfo, engine: PackInfo, world: World): SyncDeps {
  ...
  const run = async (cmd: string, args: string[], opts?: { cwd?: string }): Promise<RunResult> => {
    ...
    if (cmd === "git") {
      if (args[0] === "status") {
        return { code: 0, stdout: world.gitStatus?.[opts?.cwd ?? ""] ?? "", stderr: "" };
      }
      if (args[0] === "branch") {
        return { code: 0, stdout: world.branch ?? "main", stderr: "" };
      }
      if (args[0] === "pull") {
        const stderr = world.pullFail?.[opts?.cwd ?? ""];
        return stderr ? { code: 1, stdout: "", stderr } : { code: 0, stdout: "Already up to date.", stderr: "" };
      }
