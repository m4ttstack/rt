...
async function importAesKey(key: Uint8Array): Promise<CryptoKey> {
  return crypto.subtle.importKey("raw", bufferSource(key), "AES-GCM", false, ["encrypt", "decrypt"]);
}
...
/**
 * AES-256-GCM seal of raw bytes with an explicit IV — the seam that lets
 * tests pin a fixed vector. `seal`/`sealReply` are thin wrappers that supply
 * a random IV; callers needing determinism (tests only) call this directly.
 */
export async function sealBytes(plaintext: Uint8Array, key: Uint8Array, iv: Uint8Array): Promise<string> {
  const cryptoKey = await importAesKey(key);
  const ct = new Uint8Array(
    await crypto.subtle.encrypt({ name: "AES-GCM", iv: bufferSource(iv) }, cryptoKey, bufferSource(plaintext)),
  );
...
/** Inverse of sealBytes. Throws the raw WebCrypto/DOMException on auth failure — callers map to UserActionableError. */
export async function openBytes(ciphertextB64: string, key: Uint8Array): Promise<Uint8Array> {
  ...
  return new Uint8Array(
    await crypto.subtle.decrypt({ name: "AES-GCM", iv: bufferSource(iv) }, cryptoKey, bufferSource(ct)),
  );
}

async function sealJson(payload: unknown, key: Uint8Array): Promise<string> {
  const iv = crypto.getRandomValues(new Uint8Array(IV_BYTES));
  return sealBytes(new TextEncoder().encode(JSON.stringify(payload)), key, iv);
}

async function openJson(ciphertextB64: string, key: Uint8Array): Promise<unknown> {
  let plaintext: Uint8Array;
  try {
    plaintext = await openBytes(ciphertextB64, key);
  } catch {
  ...
  try {
    ...
    throw new UserActionableError("invite-unreadable", "invite payload is unreadable");
  }
}

export async function seal(pointer: InvitePointer, key: Uint8Array): Promise<string> {
  return sealJson(pointer, key);
}

export async function open(ciphertextB64: string, key: Uint8Array): Promise<InvitePointer> {
  return (await openJson(ciphertextB64, key)) as InvitePointer;
}

export async function sealReply(reply: { v: 1; agePublicKey: string; handle?: string }, key: Uint8Array): Promise<string> {
  return sealJson(reply, key);
}

export async function openReply(ciphertextB64: string, key: Uint8Array): Promise<{ v: 1; agePublicKey: string; handle?: string }> {
  return (await openJson(ciphertextB64, key)) as { v: 1; agePublicKey: string; handle?: string };
}
