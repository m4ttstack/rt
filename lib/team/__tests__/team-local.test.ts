...
describe("team-local record", () => {
  test("absent record reads as all-false — a permission never granted is not held", () => {
    ...
    expect(readTeamLocal(p, SLUG)).toEqual({ createdByRt: false, rtMayManageMembership: false });
  });
  ...
  test("round-trips a granted permission", () => {
    const p = fakeProbes({ home: HOME });
    writeTeamLocal(p, SLUG, { createdByRt: true, rtMayManageMembership: true });
    expect(readTeamLocal(p, SLUG)).toEqual({ createdByRt: true, rtMayManageMembership: true });
  });
