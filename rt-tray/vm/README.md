...
- Every `.mattstack` hostname in `~/.portless/routes.json` must answer over https, not only the first. Routes are fetched once, after the served-app poll passes.
- The update leg records each app's launchd job before Sparkle runs (`logs/update-before/`) and fails an app whose pid has not moved: Sparkle replaces `Contents/Helpers/<name>` at the same path, so a job still on its old process is running the deleted binary.
