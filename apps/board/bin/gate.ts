...
try {
  if (verb === 'open') {
    ...
    if (!statePath || !kind || !questions)
      throw new Error(
        'usage: gate open <state> --kind <k> --questions <json> [--context <text>]'
      );
    ...
    if (!statePath || !answers || by !== 'pane')
      throw new Error('usage: gate answer <state> --answers <json> --by pane');
