...
| !1235 | the own MR: approved, the Slack `eyes` reaction and posted logo, five resolved threads, a peer (`pat`) reviewing; the seat is an `UNREVIEWED` reviewer on it (the seat tab's "review"); plus two structured respond gates (gate-ctx): an open respond-plan answering `pat` (two threads, one blocking; the header, thread and reply cards) and an open respond-post (a post-or-hold and resolve pick per thread, one riding a pushed fix); its review-post gate is a legacy one (prose context, tier options), the prose-fallback shot |
