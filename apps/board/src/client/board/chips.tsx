...
const RESPOND_PULSING = new Set<RespondCell>([
  'triaging',
  'implementing',
  'drafting',
]);
...
function RespondBadge({
  ...
  const label =
    respond.status === 'done'
      ? respondDoneLabel(respond.posted, respond.threads)
      : RESPOND_LABEL[respond.status];
  ...
  if (respond.reportReady && onOpen) {
    return (
      <Chip
        as="button"
        {...look}
        title={`${title} — click to read the adjudication`}
        onClick={onOpen}
      >
        {label} ↗
      </Chip>
...
const DOCTOR_PULSING = new Set<DoctorStatus>([
  'diagnosing',
  'rebasing',
  'fixing',
  'watching',
...
function DoctorBadge({ doctor }: { doctor?: DoctorInfo }) {
  ...
  const label =
    (doctor.origin === 'auto' ? 'auto·' : '') + DOCTOR_LABEL[doctor.status];
...
const NUDGE_PULSING = new Set<SentNudgeInfo['display']>([
  'confirmed',
  'launched',
]);
...
function NudgedByMarker({
  ...
  return (
    <Chip
      intent="accent"
      fw={600}
      title="a peer asked you to look at this again"
    >
