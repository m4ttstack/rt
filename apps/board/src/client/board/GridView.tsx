...
import { cleanTitle, flattenStack, mrLine, statusReasons } from './format.ts';
import {
  AuthorTag,
  MetaTokens,
  onRowClick,
...
function GridView({
  ...
  const renderCard = (mr: BoardMR, depth: number) => {
    ...
    const reasons = mr.blockers?.any
      ? statusReasons(mr).split('\n').slice(1)
      : [];
    return (
      <div
        key={mr.iid}
      ...
      >
        <CopyButton
          ...
          className="tui-copy-inline tui-copy-card"
        ...
        <div
          className="tui-card-branch"
          title={`${mr.sourceBranch} → ${mr.targetBranch}`}
        >
          {mr.sourceBranch} <span className="tui-arrow">→</span>{' '}
          {mr.targetBranch}
        </div>
        <div className="tui-card-tokens">
          {showAuthor && <AuthorTag mr={mr} />} <StatusPhrase mr={mr} />{' '}
