...
describe('review-status CLI', () => {
  test('writes status and message to the given file', async () => {
    ...
    const proc = Bun.spawn(
      ['bun', 'run', CLI, p, 'reviewing', '3 issues (1 critical)'],
      { env: NO_BOARD_ENV }
    );
  ...
  test('rejects an unknown status', async () => {
    ...
    const proc = Bun.spawn(['bun', 'run', CLI, p, 'bogus'], {
      stderr: 'pipe',
      env: NO_BOARD_ENV,
    });
    const code = await proc.exited;
  ...
  test('a failed board notify does not change the exit code or the state write', async () => {
    ...
    const proc = Bun.spawn(
      ['bun', 'run', CLI, p, 'done', 'looks solid', '--outcome', 'approve'],
      { stderr: 'pipe', env: NO_BOARD_ENV }
    );
    const code = await proc.exited;
    expect(code).toBe(0);
    const state = JSON.parse(readFileSync(p, 'utf8'));
