...
const resolverPath = (dir: string) =>
  join(repoRoot, 'skills', dir, 'scripts', 'resolve-args.sh');
  ...
  for (const w of wrappers) {
    describe(w.name, () => {
      const skillMd = readFileSync(
        join(repoRoot, 'skills', w.dir, 'SKILL.md'),
        'utf8'
      );
      ...
      test('documents the resolution order: explicit --skill wins before the resolver runs', () => {
        ...
        const resolver = skillMd.indexOf(
      ...
      test('unbound slots fail loud with machine-readable errors', () => {
        ...
        const bySlot = new Map(
          out.errors.map((e: { slot: string; code: string }) => [
            e.slot,
            e.code,
          ])
        );
