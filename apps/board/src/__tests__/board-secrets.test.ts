...
describe('readBoardSecrets', () => {
  test('returns the six board-scope keys on success', async () => {
    ...
    const res = await readBoardSecrets(
      deps({ post: async () => ({ ok: true, data }) })
    );
  ...
  test("omits keys the daemon didn't return (not-configured stays quiet)", async () => {
    const res = await readBoardSecrets(
      deps({ post: async () => ({ ok: true, data: { gitlabToken: 'g' } }) })
    );
  ...
  test('a missing/unreadable api-token file points at rt daemon start, never throws', async () => {
    const res = await readBoardSecrets(
      deps({
        readApiToken: () => {
          throw new Error(
            "ENOENT: no such file or directory, open '/fake/home/.mattstack/rt/api-token'"
          );
        },
      })
    );
  ...
  test("rtCommand's own unreachable envelope also reads as daemon-down", async () => {
    const res = await readBoardSecrets(
      deps({
        post: async () => ({
          ok: false,
          error: 'rt daemon unreachable at /fake/rt.sock: connect ENOENT',
        }),
      })
    );
    expect(res.ok).toBe(false);
  ...
  test('bad-token / missing-token point at the api-token file, not daemon-down', async () => {
    const badToken = await readBoardSecrets(
      deps({ post: async () => ({ ok: false, error: 'bad-token' }) })
    );
    expect((badToken as { message: string }).message).toContain('api-token');
    expect((badToken as { message: string }).message).not.toContain(
      'daemon start'
    );

    const missingToken = await readBoardSecrets(
      deps({ post: async () => ({ ok: false, error: 'missing-token' }) })
    );
    expect((missingToken as { message: string }).message).toContain(
      'api-token'
    );
  });
  ...
  test('bad-scope (old daemon that knows the verb but not "board") points at updating rt', async () => {
    const res = await readBoardSecrets(
      deps({ post: async () => ({ ok: false, error: 'bad-scope' }) })
    );
    expect(res.ok).toBe(false);
  ...
  test('unknown command (a daemon that predates secrets:read entirely) points at updating rt', async () => {
    const res = await readBoardSecrets(
      deps({
        post: async () => ({
          ok: false,
          error: 'unknown command secrets:read',
        }),
      })
    );
    expect(res.ok).toBe(false);
  ...
  test('an unrecognized daemon-side error surfaces verbatim, not misdirected to the token file', async () => {
    const res = await readBoardSecrets(
      deps({ post: async () => ({ ok: false, error: 'internal error boom' }) })
    );
    expect(res.ok).toBe(false);
  ...
  test('never echoes the token itself in any failure message', async () => {
    const results = await Promise.all([
      readBoardSecrets(
        deps({
          readApiToken: () => {
            throw new Error('SECRET-abc123 missing');
          },
        })
      ),
      readBoardSecrets(
        deps({
          post: async () => {
            throw new Error('SECRET-abc123 unreachable');
          },
        })
      ),
    ]);
