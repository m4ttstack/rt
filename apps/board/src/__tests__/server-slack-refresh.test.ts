...
writeFileSync(
  ...
  JSON.stringify({ "board.rtRepos": [{ project: "g/p", repo: "gitlab.example.com/g/p" }] }),
);
    ...
    if (pathname === "/project-mrs:read") {
      return new Response(
        JSON.stringify({
          ok: true,
          data: {
            mrs: { "gitlab:601": { pr, fetchedAt: Date.now(), codeownerSections: [] } },
...
const proc = Bun.spawn(
  ["bun", "run", "--preload", join(import.meta.dir, "slack-api-mock-preload.ts"), join(import.meta.dir, "..", "server.ts")],
  {
    env: {
      ...process.env,
      HOME: fakeHome,
...
async function slackStatus(): Promise<{ status?: string; posted?: boolean } | undefined> {
  const data = (await (await fetch(`http://127.0.0.1:${PORT}/data.json`)).json()) as {
...
test("/slack/refresh re-resolves a fresh notfound ref the periodic sweep would skip", async () => {
  ...
  const res = await fetch(`http://127.0.0.1:${PORT}/slack/refresh`, { method: "POST" });
...
test("/slack/refresh is POST-only", async () => {
  await ready();
  expect((await fetch(`http://127.0.0.1:${PORT}/slack/refresh`)).status).toBe(405);
}, 15_000);
