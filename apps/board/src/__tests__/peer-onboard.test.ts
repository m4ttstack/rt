...
describe('createInvite', () => {
  test('composes the one-paste string from switchboard.url + returned code', async () => {
    const f = fakeFetch((url, init) => {
      ...
      return Response.json(
        { code: 'a'.repeat(32), username: 'grace', expiresAt: 1 },
        { status: 201 }
      );
    });
    const r = await createInvite('grace', {
    ...
    expect(JSON.parse(r.body)).toEqual({
      invite: `https://sb.example.app/invite/${'a'.repeat(32)}`,
    });
  });
  ...
  test('passes relay errors through as plain text; network becomes 502', async () => {
    ...
    expect(
      await createInvite('g', { url: 'https://x', adminToken: 't', fetchFn: f })
    ).toEqual({ status: 401, body: 'unauthorized' });
    const dead = (async () => {
      throw new Error('down');
    }) as unknown as typeof fetch;
    const r = await createInvite('g', {
      url: 'https://x',
      adminToken: 't',
      fetchFn: dead,
    });
...
describe('joinSwitchboard', () => {
  ...
  test('mismatch surfaces the relay body verbatim as 409 and persists nothing', async () => {
    const f = fakeFetch(
      () =>
        new Response('this invite is for a different board handle', {
          status: 409,
        })
    );
    let persisted = false;
