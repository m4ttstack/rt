...
describe('summarizeThreads', () => {
  ...
  test('marks a reviewer thread the author last replied to as replied', () => {
    const d = detail([
      { notes: [note('reviewer'), note('dorothy', { body: 'done' })] },
    ]);
...
describe('summarizeDiscussions', () => {
  ...
  test('a later author general comment flips an awaiting thread to replied', () => {
    const d = detail([
      {
        notes: [
          note('reviewer', { body: 'please fix', at: '2026-07-10T00:00:00Z' }),
        ],
      }, // awaiting
      {
        notes: [
  ...
  test('an author general comment BEFORE the thread does not flip it', () => {
    const d = detail([
      ...
      }, // earlier general comment
      {
        notes: [
          note('reviewer', { body: 'new issue', at: '2026-07-10T05:00:00Z' }),
        ],
      }, // later awaiting thread
    ]);
    const { threads } = summarizeDiscussions(d, AUTHOR);
    expect(threads[0]!.status).toBe('awaiting');
  });
    ...
    const d = detail([
      {
        notes: [
          note('bot', {
            resolvable: false,
            body: "<!-- linear-linkback --> <details><summary><a href='https://linear.app/x'>ACME-1</a></summary>huge blob</details>",
          }),
        ],
      },
    ...
    const d = detail([
      ...
      }, // awaiting
      {
        notes: [
          note('dorothy', {
            resolvable: false,
            body: '<!-- linear-linkback --> <details>x</details>',
            at: '2026-07-10T05:00:00Z',
          }),
        ],
      },
  ...
  test('isBotUsername flags service accounts and delimited bot names, not real people', () => {
    ...
    expect(
      isBotUsername(
        'service_account_group_6451920_367c36ac02936601931578db2adf4465'
      )
    ).toBe(true);
  ...
  test("config botUsernames hides a named bot's general comments (username or display name)", () => {
    ...
    expect(
      summarizeDiscussions(d, AUTHOR, ['mr_mr']).comments.map(c => c.body)
    ).toEqual(['real']);
    expect(
      summarizeDiscussions(d, AUTHOR, ['MR_MR']).comments.map(c => c.body)
    ).toEqual(['real']); // case-insensitive
    expect(
      summarizeDiscussions(d, AUTHOR, []).comments.map(c => c.body)
    ).toEqual(['auto note', 'real']); // not listed → kept
  });
  ...
  test('drops general comments authored by a bot', () => {
    const d = detail([
      {
        notes: [
          note('release-bot', { resolvable: false, body: 'pipeline passed' }),
        ],
      },
      { notes: [note('dorothy', { resolvable: false, body: 'real comment' })] },
    ]);
    expect(summarizeDiscussions(d, AUTHOR).comments.map(c => c.body)).toEqual([
      'real comment',
    ]);
  });
  ...
  test('summarizeThreads still returns just the (flipped) threads', () => {
    ...
    expect(summarizeThreads(d, AUTHOR)).toEqual(
      summarizeDiscussions(d, AUTHOR).threads
    );
  });
});
...
describe('latch exclusion', () => {
  ...
  test('a spent latch is not counted as a resolved thread', () => {
    const d = detail([
      {
        notes: [
          note('reviewer', { body: spentLatchBody(IMG), resolved: true }),
        ],
      },
    ]);
    const { threads } = summarizeDiscussions(d, AUTHOR);
  ...
  test('a latch is not counted as a general comment either', () => {
    const d = detail([
      {
        notes: [
          note('reviewer', { body: armedLatchBody(IMG), resolvable: false }),
        ],
      },
    ]);
    const { comments } = summarizeDiscussions(d, AUTHOR);
