...
describe('decide', () => {
  test('clean state dispatches', () => {
    expect(decide(edge, emptyMrMemory('2026-08-08'), 0, cfg, NOW)).toEqual({
      action: 'dispatch',
      reason: 'edge:pipeline-red',
    });
  });
  ...
  test('budget exhausted escalates once, then skips', () => {
    ...
    expect(decide(edge, escalated, 0, cfg, NOW)).toEqual({
      action: 'skip',
  ...
  test('cooldown skips', () => {
    const m = {
      ...
      lastDispatchAt: NOW - 10 * 60_000,
    }; // 10 min ago < 30 min cooldown
    expect(decide(edge, m, 0, cfg, NOW)).toEqual({
      action: 'skip',
      reason: 'cooldown',
    });
    const cooled = {
      ...emptyMrMemory('2026-08-08'),
      lastDispatchAt: NOW - 31 * 60_000,
    };
    expect(decide(edge, cooled, 0, cfg, NOW).action).toBe('dispatch');
  });

  test('global concurrency cap skips', () => {
    expect(decide(edge, emptyMrMemory('2026-08-08'), 2, cfg, NOW)).toEqual({
      action: 'skip',
