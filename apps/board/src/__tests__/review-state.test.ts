...
describe('pruneReviewStates', () => {
  ...
  test('keeps states whose MR is on the board, deletes the rest (and their reports)', () => {
    ...
    writeReviewState(reviewFilePath(OTHER, dir), {
      mrUrl: OTHER,
      iid: 1,
      status: 'done',
    });
    writeFileSync(
      reviewReportPath(reviewFilePath(OTHER, dir)),
      '# review OTHER'
    );
    ...
    expect(existsSync(reviewReportPath(reviewFilePath(OTHER, dir)))).toBe(
      false
    ); // report gone too
  });
});
...
describe('review report', () => {
  test('reportPath is the state path with a .md extension', () => {
    expect(reviewReportPath(reviewFilePath(URL_A, dir))).toBe(
      reviewFilePath(URL_A, dir).replace(/\.json$/, '.md')
    );
  ...
  test('readReviewReport returns the saved markdown, or null when absent', () => {
    ...
    writeFileSync(
      reviewReportPath(reviewFilePath(URL_A, dir)),
      '# Review\n\nlooks good'
    );
