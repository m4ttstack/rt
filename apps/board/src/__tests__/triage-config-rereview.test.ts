...
describe('loadReReviewConfig', () => {
  test('an owned board.reReview { enabled: false } turns the latch off', () => {
    expect(
      loadReReviewConfig(fakeResolve({ 'board.reReview': { enabled: false } }))
    ).toEqual({ enabled: false });
  });

  test('an owned board.reReview { enabled: true } turns it on', () => {
    expect(
      loadReReviewConfig(fakeResolve({ 'board.reReview': { enabled: true } }))
  ...
  test('a malformed value names the settings key, not config.json', () => {
    expect(() =>
      loadReReviewConfig(fakeResolve({ 'board.reReview': { enabled: 'yes' } }))
    ).toThrow(/settings key "board.reReview".*enabled.*boolean/);
  });
});
