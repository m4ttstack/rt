...
describe('parsers', () => {
  ...
  test('parseTabCreate pulls tab + pane ids', () => {
    expect(parseTabCreate(TAB_CREATE)).toEqual({
      tabId: 'w40:t3',
      paneId: 'w40:p7',
      workspaceId: 'w40',
    });
  ...
  test('parseWorkspaceCreate pulls workspace + pane ids', () => {
    expect(parseWorkspaceCreate(WS_CREATE)).toEqual({
      workspaceId: 'w41',
      tabId: 'w41:t1',
      paneId: 'w41:p1',
    });
...
describe('command builders', () => {
  ...
  test('doctorPrompt appends the operator note after every flag', () => {
    const p = doctorPrompt({
      mrUrl: 'https://x/mr/1',
      statePath: '/s/1.json',
      statusBin: '/b/doctor-status.ts',
      tier: 'api',
      note: 'the lint job is the real blocker',
    });
    expect(p).toBe(
  ...
  test('parseLaunchNote accepts absent and blank notes as undefined', () => {
    ...
    expect(parseLaunchNote({ note: '' })).toEqual({
      ok: true,
      note: undefined,
    });
    expect(parseLaunchNote({ note: '   ' })).toEqual({
  ...
  test('parseLaunchNote trims and returns a real note', () => {
    expect(parseLaunchNote({ note: '  look at !42 too  ' })).toEqual({
      ok: true,
      note: 'look at !42 too',
    });
  });
  test('parseLaunchNote rejects non-strings and over-long notes', () => {
    expect(parseLaunchNote({ note: 42 })).toEqual({
      ok: false,
      error: 'note must be a string',
    });
    expect(parseLaunchNote({ note: 'x'.repeat(2001) })).toEqual({
      ok: false,
      error: 'note too long (max 2000 chars)',
    });
    expect(parseLaunchNote({ note: 'x'.repeat(2000) })).toEqual({
      ok: true,
      note: 'x'.repeat(2000),
    });
  });
  ...
  test('buildResumePaneCommand with no prompt drops into an interactive resume', () => {
    expect(buildResumePaneCommand('/repo', 'sess-1')).toBe(
      "cd '/repo' && claude --resume 'sess-1'"
    );
  });
...
function fakeAgentIo(): {
  ...
  const io: AgentIo = {
    ...
    agentResume: async () => {
      throw new Error('agentResume not used by launchReview/Respond/Doctor');
    },
  };
...
describe('launchReview / launchRespond / launchDoctor (rt agent)', () => {
    ...
    await launchReview(
      {
        ...
        workspaceLabel: 'reviews',
        statePath: '/s/1.json',
        note: 'skip the vendored files',
      },
      io
    );
    expect(startCalls[0]!.prompt).toContain(
      'Operator note (from the human who launched this pane): skip the vendored files'
    );
  });
  ...
  test('launchDoctor adds --skill-path but keeps launching the /board:doctor wrapper, tier intact', async () => {
    ...
    const resolvePath = async () =>
      '/cache/acme/attachments/board-doctor-api/SKILL.md';
    await launchDoctor(
      {
        mrUrl: 'https://x/mr/1',
        iid: 4821,
...
describe('doctorPrompt tier flags', () => {
  test('emits --tier, --fix-classes, and --draft-bin when given', () => {
    const p = doctorPrompt({
      ...
      statePath: '/s',
      statusBin: statusBinPath(),
      skill: 'team:doctor-api',
      tier: 'api',
      fixClasses: ['retry-flake', 'inherited-note-draft'],
...
describe('dispatchPrompt (wrapper hop stays; --skill-path rides alongside --skill)', () => {
  ...
  test('adds --skill-path after --skill when resolution succeeds, keeping the wrapper hop', async () => {
    const resolvePath = async (name: string) =>
      name === 'acme:board-review'
        ? '/cache/acme/skills/board-review/SKILL.md'
        : null;
  ...
  test('keeps the operator note as a trailing paragraph after --skill-path', async () => {
    ...
    const prompt = await dispatchPrompt(
      'board:review',
      { ...baseOpts, note: 'focus on the migration files' },
      resolvePath
    );
