...
describe('composeAgentCommand', () => {
  ...
  test('the word claude never follows the cswap --, per docs/configuration.md', () => {
    expect(composeAgentCommand({ account: 'a@b.c', model: 'm' })).not.toContain(
      '-- claude'
    );
  });
  ...
  test('model without an account flags plain claude rather than dropping the model', () => {
    expect(composeAgentCommand({ model: 'claude-opus-5' })).toBe(
      "claude --model 'claude-opus-5'"
    );
    expect(composeAgentCommand({ effort: 'max' })).toBe(
      "claude --effort 'max'"
    );
  });
...
describe('loadAgentSettings', () => {
  ...
  test('an empty registry leaves every field undefined -- no shell composition happens here', () => {
    expect(loadAgentSettings(registryOf({}))).toEqual({
      account: undefined,
      model: undefined,
      effort: undefined,
    });
  ...
  test('a resolver that throws on every key degrades every field to undefined, not a crash', () => {
    ...
    expect(loadAgentSettings(throwing)).toEqual({
      account: undefined,
      model: undefined,
      effort: undefined,
    });
...
describe('loadConfig reads the board.agent.* keys', () => {
  ...
  test('the retired board.claudeCommand is not consulted, even when a store still holds it', () => {
    const cfg = configWith({
      'board.claudeCommand': "cswap run 'old' --share-history --",
    });
    expect(cfg.claudeCommand).toBe('');
  });
  ...
  test('a file-config claudeCommand still wins as the verbatim escape hatch', () => {
    ...
    expect(loadConfigFrom(path, registryOf({})).claudeCommand).toBe(
      'my-wrapper --'
    );
  });
  ...
  test('the store beats the file when both are set', () => {
    ...
    const cfg = loadConfigFrom(
      path,
      registryOf({ 'board.agent.account': 'a@b.c' })
    );
