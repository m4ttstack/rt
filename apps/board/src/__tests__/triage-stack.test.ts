...
describe('chainOf', () => {
  test('an MR that targets the default branch has no ancestors and nothing unresolved', () => {
    const solo = mr();
    expect(chainOf([solo], solo.mrUrl)).toEqual({
      ancestors: [],
      unresolvedParentBranch: null,
    });
  });
  ...
  test('walks a three-deep stack nearest-parent-first', () => {
    ...
    expect(chainOf([a, b, c], c.mrUrl).ancestors.map(x => x.iid)).toEqual([
      2, 1,
    ]);
  });
  ...
  test('a stacked MR whose parent is outside the board window reports the unresolved branch', () => {
    const orphan = mr({
      ...
      iid: 2,
      sourceBranch: 'c',
      targetBranch: 'invisible-parent',
      isStacked: true,
    });
    expect(chainOf([orphan], orphan.mrUrl)).toEqual({
      ancestors: [],
      unresolvedParentBranch: 'invisible-parent',
    });
  });
  ...
  test('an identical branch name in a different project never links a stack', () => {
    const other = mr({
      mrUrl: 'https://gitlab.com/other/repo/-/merge_requests/1',
      iid: 1,
      sourceBranch: 'p',
    });
    const child = mr({
      mrUrl: 'https://gitlab.example.com/acme/webapp/-/merge_requests/2',
      iid: 2,
      sourceBranch: 'c',
      targetBranch: 'p',
      isStacked: true,
  ...
  test('an unknown mrUrl yields an empty chain', () => {
    expect(chainOf([mr()], 'https://x/mr/999')).toEqual({
      ancestors: [],
      unresolvedParentBranch: null,
    });
  });
