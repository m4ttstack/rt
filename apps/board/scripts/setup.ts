    ...
    const server = Bun.serve({
      ...
      async fetch(req) {
        ...
        if (err) {
          ...
          return html(
            `<h1>Authorization failed</h1><p>${escape(err)}</p>`,
            400
          );
        }
        ...
        try {
          ...
          reject(e);
          return html(
            `<h1>Token exchange failed</h1><p>${escape((e as Error).message)}</p>`,
            500
          );
        }
      },
...
async function main() {
  ...
  if (!gitlabToken) {
    console.error(
      'A GitLab token is required. Create one at https://gitlab.com/-/user_settings/personal_access_tokens'
    );
    process.exit(1);
  }
  ...
  const skipSlack = (ask('Set up Slack integration now? (Y/n)', 'Y') || 'Y')
    .toLowerCase()
    .startsWith('n');
  if (!skipSlack) {
    if (env.SLACK_TOKEN) {
      const reuse = (
        ask('An existing SLACK_TOKEN was found. Reuse it? (Y/n)', 'Y') || 'Y'
      )
        .toLowerCase()
        .startsWith('n');
    ...
    if (!env.SLACK_TOKEN) {
      const teamSlack = getSetting<{ slack?: { clientId?: string } }>(
      ...
      const clientSecret = ask(
        ...
        env.SLACK_CLIENT_SECRET || process.env.SLACK_CLIENT_SECRET
      );
      if (!clientId || !clientSecret) {
        console.error(
          'Slack setup needs a client id + secret from your own Slack app (see README). Skipping Slack.'
        );
      } else {
        ...
        try {
          ...
          console.error(
            'You can re-run `bun run setup` any time to try again. Continuing without a Slack token.'
          );
        }
      }
    }
  ...
  setSetting(
    ...
    {
      review: reviewCwd,
      respond: existingConfig?.respondCwd ?? '',
      doctor: existingConfig?.doctorCwd ?? '',
    },
    'machine'
  );
  console.log(
