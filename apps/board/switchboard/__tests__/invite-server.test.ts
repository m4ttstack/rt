...
function handler(db = new Database(':memory:')) {
  ...
  return {
    store,
    fetch: makeFetchHandler(store, ADMIN, () => 5000, new TeamInviteStore(db)),
  };
}
...
describe('POST /invites', () => {
  ...
  test('401 without admin bearer, 400 on bad body', async () => {
    const { fetch } = handler();
    expect(
      (await fetch(req('/invites', { method: 'POST', body: '{}' }))).status
    ).toBe(401);
    expect(
      (
        await fetch(
          req('/invites', { method: 'POST', headers: asAdmin, body: '{}' })
        )
      ).status
    ).toBe(400);
  });
...
describe('POST /invites/redeem', () => {
  ...
  test('409 mismatch does not consume; 404 unknown; 410 expired; bodies never name the bound handle', async () => {
    const { store, fetch } = handler();
    const inv = store.createInvite('grace', 1000);
    const mism = await fetch(
      req('/invites/redeem', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ code: inv.code, username: 'bob' }),
      })
    );
    ...
    const ok = await fetch(
      req('/invites/redeem', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ code: inv.code, username: 'grace' }),
      })
    );
    ...
    const unknown = await fetch(
      req('/invites/redeem', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ code: 'f'.repeat(32), username: 'x' }),
      })
    );
    ...
    const expired = await fetch(
      req('/invites/redeem', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ code: expInv.code, username: 'bob' }),
      })
    );
...
describe('GET /boards', () => {
  test('401 non-admin; 200 admin list', async () => {
    ...
    const res = await fetch(
      req('/boards', { headers: { authorization: `Bearer ${ADMIN}` } })
    );
    expect(res.status).toBe(200);
    const body = (await res.json()) as {
      boards: Array<{ username: string; createdAt: number }>;
    };
