...
function setup(startAt = 1_000_000) {
  ...
  return { db, store, teamStore, call, tick: (ms: number) => { clock += ms; }, at: () => clock };
}
...
async function mint(
  ...
  overrides: { id?: string; ciphertext?: string; expiresAt?: string; ip?: string } = {},
) {
  ...
  return { res, body: res.status === 201 ? (await res.json()) as { id: string; creatorSecret: string } : null };
}
...
describe("the relay's storage tells an operator nothing", () => {
  ...
  test("team_invites has exactly the columns the spec allows", () => {
    const { db } = setup();
    const columns = db.query<{ name: string }, []>(`PRAGMA table_info(team_invites)`).all().map((c) => c.name);
  ...
  test("the creator secret is stored only as a hash", async () => {
    ...
    const stored = db.query<{ creator_secret_hash: string }, []>(`SELECT creator_secret_hash FROM team_invites`).all();
...
describe("rate limits on the unauthenticated surface", () => {
  ...
  test("the limit is per source address and lifts with time", async () => {
    ...
    expect((await mint(call, { id: idAt(20), ip: "198.51.100.4" })).res.status).toBe(201);
    tick(60_001);
    expect((await mint(call, { id: idAt(30), expiresAt: iso(1_000_000 + 48 * HOUR) })).res.status).toBe(201);
  });
  ...
  test("guessing a creator secret is throttled per invite", async () => {
    ...
    await mint(call);
    for (let i = 0; i < 10; i++) {
      expect((await call(`/v1/invites/${ID}/reply`, { token: `guess-${i}`, method: "GET" })).status).toBe(401);
    }
    expect((await call(`/v1/invites/${ID}/reply`, { token: "guess-11", method: "GET" })).status).toBe(429);
    expect((await call(`/v1/invites/${ID}`, { token: "guess-11", method: "DELETE" })).status).toBe(429);
  });
  ...
  test("a throttled invite still serves its real creator", async () => {
    ...
    for (let i = 0; i < 12; i++) await call(`/v1/invites/${ID}/reply`, { token: `guess-${i}`, method: "GET" });
    const read = await call(`/v1/invites/${ID}/reply`, { token: creatorSecret, method: "GET" });
    expect(read.status).toBe(200);
    expect((await read.json() as { blob: string }).blob).toBe("sealed-age-key");
  });
});
...
describe("POST /v1/invites", () => {
  ...
  test("a second post to a live id is 409 and leaves the original record untouched", async () => {
    ...
    expect((await read.json() as { ciphertext: string }).ciphertext).toBe("sealed-team-pointer");
    const reply = await call(`/v1/invites/${ID}/reply`, { token: first.creatorSecret, method: "GET" });
    expect(reply.status).toBe(404);
  });

  test("rejects an id that is not 32 lowercase hex", async () => {
    const { call } = setup();
    for (const bad of ["short", ID.toUpperCase(), `${ID}f`, "zzzz56789abcdef0123456789abcdef0"]) {
  ...
  test("rejects a missing ciphertext and a missing, unparseable, or past expiry", async () => {
    const { call } = setup();
    const bad = async (body: unknown) => (await call("/v1/invites", { body })).status;
    expect(await bad({ id: ID, expiresAt: iso(1_000_000 + HOUR) })).toBe(400);
    expect(await bad({ id: ID, ciphertext: 7, expiresAt: iso(1_000_000 + HOUR) })).toBe(400);
    expect(await bad({ id: ID, ciphertext: "c" })).toBe(400);
    expect(await bad({ id: ID, ciphertext: "c", expiresAt: "not-a-date" })).toBe(400);
    expect(await bad({ id: ID, ciphertext: "c", expiresAt: iso(1_000_000 - HOUR) })).toBe(400);
  });
...
describe("GET /v1/invites/:id", () => {
  ...
  test("a redeemed invite is 410", async () => {
    const { call } = setup();
    await mint(call);
    expect((await call(`/v1/invites/${ID}/redeem`, { method: "POST" })).status).toBe(200);
    expect((await call(`/v1/invites/${ID}`)).status).toBe(410);
  });
});
...
describe("POST /v1/invites/:id/redeem", () => {
  ...
  test("eight concurrent redeems still yield exactly one winner", async () => {
    ...
    await mint(call);
    const results = await Promise.all(
      Array.from({ length: 8 }, () => call(`/v1/invites/${ID}/redeem`, { method: "POST" })),
    );
  ...
  test("unknown is 404 and expired is 410", async () => {
    const { call, tick } = setup();
    expect((await call(`/v1/invites/${ID}/redeem`, { method: "POST" })).status).toBe(404);
    await mint(call, { expiresAt: iso(1_000_000 + HOUR) });
    tick(HOUR + 1);
    expect((await call(`/v1/invites/${ID}/redeem`, { method: "POST" })).status).toBe(410);
  });
});
...
describe("POST /v1/invites/:id/reply", () => {
  test("the joiner posts a sealed key without holding any credential", async () => {
    const { call } = setup();
    const { creatorSecret } = (await mint(call)).body!;
    expect((await call(`/v1/invites/${ID}/reply`, { body: { blob: "sealed-age-key" } })).status).toBe(200);
    const read = await call(`/v1/invites/${ID}/reply`, { token: creatorSecret, method: "GET" });
  ...
  test("a second reply is 409 and the first blob survives", async () => {
    ...
    expect((await call(`/v1/invites/${ID}/reply`, { body: { blob: "impostor" } })).status).toBe(409);
    const read = await call(`/v1/invites/${ID}/reply`, { token: creatorSecret, method: "GET" });
  ...
  test("unknown is 404, expired is 410, oversize is 413, missing blob is 400", async () => {
    const { call, tick } = setup();
    expect((await call(`/v1/invites/${ID}/reply`, { body: { blob: "b" } })).status).toBe(404);
    await mint(call, { expiresAt: iso(1_000_000 + 2 * HOUR) });
    expect((await call(`/v1/invites/${ID}/reply`, { body: {} })).status).toBe(400);
    expect((await call(`/v1/invites/${ID}/reply`, { body: { blob: "x".repeat(64 * 1024 + 1) } })).status).toBe(413);
    tick(2 * HOUR + 1);
    expect((await call(`/v1/invites/${ID}/reply`, { body: { blob: "b" } })).status).toBe(410);
  });
});
...
describe("GET /v1/invites/:id/reply", () => {
  test("no reply yet is 404, which rt polls on", async () => {
    const { call } = setup();
    const { creatorSecret } = (await mint(call)).body!;
    expect((await call(`/v1/invites/${ID}/reply`, { token: creatorSecret, method: "GET" })).status).toBe(404);
  });
  ...
  test("a wrong or missing creator secret cannot read the reply", async () => {
    ...
    await mint(call);
    await call(`/v1/invites/${ID}/reply`, { body: { blob: "sealed-age-key" } });
    expect((await call(`/v1/invites/${ID}/reply`, { method: "GET" })).status).toBe(401);
    expect((await call(`/v1/invites/${ID}/reply`, { token: "wrong", method: "GET" })).status).toBe(401);
  });
});
...
describe("DELETE /v1/invites/:id", () => {
  test("the creator revokes and the invite is gone for everyone", async () => {
    const { call } = setup();
    const { creatorSecret } = (await mint(call)).body!;
    expect((await call(`/v1/invites/${ID}`, { token: creatorSecret, method: "DELETE" })).status).toBe(204);
    expect((await call(`/v1/invites/${ID}`)).status).toBe(410);
    expect((await call(`/v1/invites/${ID}/redeem`, { method: "POST" })).status).toBe(410);
  });
  ...
  test("a wrong secret is 401 and the invite survives", async () => {
    const { call } = setup();
    await mint(call);
    expect((await call(`/v1/invites/${ID}`, { method: "DELETE" })).status).toBe(401);
    expect((await call(`/v1/invites/${ID}`, { token: "wrong", method: "DELETE" })).status).toBe(401);
    expect((await call(`/v1/invites/${ID}`)).status).toBe(200);
  });
  ...
  test("an invite already gone is 404, which rt treats as success", async () => {
    ...
    expect((await call(`/v1/invites/${ID}`, { token: creatorSecret, method: "DELETE" })).status).toBe(404);
    const unknown = "ffffffffffffffffffffffffffffffff";
    expect((await call(`/v1/invites/${unknown}`, { token: creatorSecret, method: "DELETE" })).status).toBe(404);
  });
});
