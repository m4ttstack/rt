...
describe('transient classification', () => {
  ...
  it('reads Retry-After in seconds, and ignores a missing or unparseable one', () => {
    const withHeader = (v: string) =>
      new Response('', { status: 429, headers: { 'retry-after': v } });
...
describe('GraphQL transport (Linear)', () => {
  it('retries a dropped connection and succeeds on the next attempt', async () => {
    ...
    await expect(linearRequest('key', 'query { ok }', {})).resolves.toEqual({
      ok: true,
    });
    expect(calls).toBe(2);
  });
  ...
  it('does not retry a query error ... a bad field is bad on every attempt', async () => {
    ...
    vi.stubGlobal('fetch', async () => {
      calls += 1;
      return new Response(
        JSON.stringify({ errors: [{ message: 'no field x' }] }),
        { status: 200 }
      );
    });

    await expect(linearRequest('key', 'query { x }', {})).rejects.toThrow(
      /no field x/
    );
    expect(calls).toBe(1);
  });
  ...
  it('does not retry a 404 ... a missing resource stays missing', async () => {
    ...
    await expect(linearRequest('key', 'query { ok }', {})).rejects.toThrow(
      /Linear HTTP 404/
    );
    expect(calls).toBe(1);
  });
