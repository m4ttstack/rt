...
function validResponse(): LeaderboardResponse {
  const a = makeUser(
    'a',
    makeMetrics({ mrsMerged: 10, revertRate: 0.0, codingDays: 5 }),
    { isCurrentUser: true }
  );
  const b = makeUser(
    'b',
    makeMetrics({ mrsMerged: 4, revertRate: 0.3, codingDays: 2 })
  );
  const users = [a, b];
...
const codes = (res: LeaderboardResponse) =>
  validateLeaderboard(res).issues.map(i => i.code);
...
describe('validateLeaderboard', () => {
  ...
  it('warns (not errors) when a metric is uniform across users', () => {
    ...
    expect(
      report.issues.some(
        i => i.severity === 'warn' && i.code === 'metric_uniform'
      )
    ).toBe(true);
  });
