...
import type {
  RequestIO,
  SourceProvider,
} from '../src/server/source/provider.js';
...
describe('scanProject', () => {
  it('passes projectPaths/updatedAfter/signal and stamps scannedAt on every row', async () => {
    ...
    const rows = await scanProject(provider, 'g/p', '2026-05-01T00:00:00Z', {
...
describe('fetchMetrics', () => {
  it('returns null when glance returns null (MR gone)', async () => {
    const { provider } = makeFakeProvider({
      fetchMergeRequestMetrics: async () => null,
    });
  ...
  it('maps the metrics and forwards signal', async () => {
    ...
    const { provider, calls } = makeFakeProvider({
      fetchMergeRequestMetrics: async () => metrics,
    });

    const result = await fetchMetrics(provider, 'g/p', 5, {
      signal: controller.signal,
    });
    ...
    expect(calls).toEqual([
      {
        method: 'fetchMergeRequestMetrics',
        args: ['g/p', 5, { signal: controller.signal }],
      },
    ]);
  });
});
...
describe('fetchPipelinesFor', () => {
  it('passes username/updatedAfter/updatedBefore and maps status through unchanged', async () => {
    ...
    const { provider, calls } = makeFakeProvider({
      fetchProjectPipelines: async () => [summary],
    });

    const rows = await fetchPipelinesFor(provider, 'g/p', 'ada', window, {
      signal: controller.signal,
    });
...
describe('fetchProjectRef', () => {
  it("passes the signal through and returns the provider's project ref verbatim", async () => {
    ...
    const ref = await fetchProjectRef(provider, 'g/p', {
      signal: controller.signal,
    });

    expect(calls).toEqual([
      { method: 'fetchProject', args: ['g/p', { signal: controller.signal }] },
    ]);
...
describe('resolveIdentity', () => {
  it('resolves an exact username match and stamps fetchedAt', async () => {
    ...
    const { provider, calls } = makeFakeProvider({
      restRequest: async () =>
        new Response(
          JSON.stringify([{ id: 7, username: 'ada', name: 'Ada Lovelace' }]),
          { status: 200 }
        ),
    });

    const identity = await resolveIdentity(provider, 'ada', {
      signal: controller.signal,
    });
