...
describe('MetricCards', () => {
  it("ranks each metric's card independently, by that metric's own server-computed rank", () => {
    ...
    const aliceMetrics = buildMetrics({
      mrsMerged: { value: 9 },
      additions: { value: 100 },
    });
    aliceMetrics.mrsMerged.rank = 2;
    aliceMetrics.additions.rank = 1;
    const bobMetrics = buildMetrics({
      mrsMerged: { value: 14 },
      additions: { value: 50 },
    });
    ...
    const data = buildResponse([
      buildUser('alice', aliceMetrics),
      buildUser('bob', bobMetrics),
    ]);
    ...
    expect(
      mrsCard.getAllByText(/^(alice|bob)$/).map(el => el.textContent)
    ).toEqual(['bob', 'alice']);

    const additionsCard = within(screen.getByTestId('metric-card-additions'));
    expect(
      additionsCard.getAllByText(/^(alice|bob)$/).map(el => el.textContent)
    ).toEqual(['alice', 'bob']);
  });
  ...
  it("navigates to that user's metric detail route when a ranked row is clicked", () => {
    ...
    fireEvent.click(
      within(screen.getByTestId('metric-card-mrsMerged')).getByText('alice')
    );
  ...
  it("renders a delta badge with the trend's sign when trend is on, and omits it when off", () => {
    ...
    const { unmount } = renderWithProviders(
      <MetricCards data={withTrend} trend />
    );
    const badge = within(screen.getByTestId('metric-card-mrsMerged')).getByText(
      /▲ 4/
    );
    ...
    expect(
      within(screen.getByTestId('metric-card-mrsMerged')).queryByText(/▲ 4/)
  ...
  it("excludes unresolved users and users with no value for that metric from a card's ranking", () => {
    const data = buildResponse([
      buildUser('alice', buildMetrics({ mrsMerged: { value: 9 } })),
      buildUser('bob', buildMetrics({ mrsMerged: { value: 3 } }), {
        resolved: false,
      }),
    ]);
