...
export async function linearRequest<T>(
  ...
  const json = await withRetry<GqlResponse<T>>(
    async attemptSignal => {
      let res: Response;
      try {
        ...
        throw asRetryable(
          (err as Error).name === 'AbortError'
            ? err
            : new LinearApiError(
                `Linear request failed: ${(err as Error).message}`
              ),
          signal
        );
      }
      ...
      if (!res.ok) {
        ...
        const error = new LinearApiError(
          `Linear HTTP ${res.status}: ${text}`,
          res.status
        );
        if (isTransientStatus(res.status))
          throw new RetryableError(error, retryAfterMs(res));
        throw error;
      }
