...
import {
  eligibleForLinearDiscovery,
  resolveLinearTickets,
} from '../linear/fetch.js';
import {
  fetchMetrics,
  fetchPipelinesFor,
  fetchProjectRef,
...
export async function runRefresh(
  ...
  const existingIdentities = new Map(
    store.identities(rosterUsernames).map(i => [i.username, i])
  );
  ...
  await mapLimit(
    ...
    (done, total) =>
      report({ phase: 'users', label: 'Resolving users', done, total })
  );
  ...
  report({
    ...
    total: settings.projects.length,
  });
  await mapLimit(
    settings.projects,
    CONCURRENCY,
    async projectPath => {
      ...
      try {
        const rows = await scanProject(source, projectPath, updatedAfter, {
          signal,
        });
        ...
        warnings.push({
          ...
          message: `MRs for ${projectPath}: ${(err as Error).message}`,
        });
      }
    },
    (done, total) =>
      report({ phase: 'mrs-list', label: 'Scanning projects', done, total })
  );
  ...
  const eligibleRows = indexRows.filter(
    r =>
      (r.authorUsername != null && rosterSet.has(r.authorUsername)) ||
  ...
  try {
    await mapLimit(
      ...
      async row => {
        signal?.throwIfAborted();
        try {
          const detail = await fetchMetrics(source, row.projectPath, row.iid, {
            signal,
          });
          if (detail) {
  ...
  await mapLimit(
    ...
    async ({ projectPath, username }) => {
      signal?.throwIfAborted();
      try {
        const rows = await fetchPipelinesFor(
          source,
          projectPath,
          username,
          window,
    ...
    (done, total) =>
      report({ phase: 'pipelines', label: 'Fetching pipelines', done, total })
  );
    ...
    try {
      ...
      if (ref) {
        ...
        warnings.push({
          ...
          message: `Project id for ${projectPath}: not found`,
        });
      }
    } catch (err) {
      if (isAbort(err)) throw err;
  ...
  report({
    phase: 'pushes',
    label: 'Fetching push events',
    done: 0,
    total: rosterUsernames.length,
  });
  await mapLimit(
    rosterUsernames,
    CONCURRENCY,
    async username => {
      ...
      try {
        ...
        const rows = await fetchPushesFor(
          source,
          `gitlab:user:${identity.userId}`,
          username,
          window,
          { signal }
        );
        const scoped = rows.filter(
          r => r.repositoryId != null && scopedProjectIds.has(r.repositoryId)
        );
    ...
    (done, total) =>
      report({ phase: 'pushes', label: 'Fetching push events', done, total })
  );
  ...
  const ticketSources = buildFetchResult(
    store,
    window,
    rosterUsernames
  ).mrs.filter(
    m =>
      eligibleKeySet.has(mrKey(m.projectPath, m.iid)) &&
      eligibleForLinearDiscovery(m)
  );
  const linearIssues = await resolveLinearTickets(
    env.linearApiKey,
    ticketSources,
    warnings,
    rosterUsernames,
