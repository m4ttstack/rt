...
if (!token || !zoneId || !domain) {
  console.error(
    'CF_TOKEN, CF_ZONE, CF_DOMAIN are required. Run this yourself, the token never goes through an agent.'
  );
  process.exit(2);
}
...
try {
  ...
  await cf.sync('local-scratch', host, {
    mode: 'emails',
    emails: ['verify@local.invalid'],
  });
  ...
  if (!gated) {
    failed = true;
    console.error(
      'FAIL: the hostname did not answer with an Access login redirect.'
    );
    console.error(
      "If DNS for the wildcard is not in place yet, run this after Task 4.4's bind on the real zone."
    );
  }
} catch (err) {
  failed = true;
  ...
  try {
    await cf.remove(host);
    console.log('teardown clean.');
  } catch (err) {
    console.error('TEARDOWN FAILED, remove it in the CF dashboard:', host, err);
    failed = true;
  }
}
console.log(
  failed
    ? 'RESULT: FAIL'
    : 'RESULT: PASS, token scope is sufficient for Access apps + policies.'
);
