...
async function openAccess(page: Page, name: string): Promise<void> {
  ...
  await page
    ...
    .locator('button')
    .click();
  await page.waitForSelector('[data-part="drawer-title"]', {
    hasText: 'access',
  });
}
...
test('root: password value set/not set, sign-in toggle, who hidden until sign-in is on', async () => {
  await withBoard(async page => {
    ...
    expect(
      await accessNav(page, 'password')
        .locator('[data-part="listgroup-value"]')
        .textContent()
    ).toBe('set');
    const toggle = page.locator(
      '[data-part="listgroup-toggle"] [data-part="switch-control"]'
    );
    expect(await toggle.isChecked()).toBe(true);
    expect(
      await accessNav(page, 'who')
        .locator('[data-part="listgroup-value"]')
        .textContent()
    ).toBe('2 people');
    ...
    expect(
      await page
        .locator('[data-part="listgroup-footer"]', {
...
test('password: remove -- no confirm, PUTs password:null immediately', async () => {
  await withBoard(async page => {
    ...
    const danger = page.locator('button[data-intent="bad"]', {
      hasText: 'remove password',
    });
...
test('who: mode switch clears entries; entries add/remove; save disabled on empty', async () => {
  await withBoard(async page => {
    ...
    await page
      .locator('[role="radio"]', { hasText: 'anyone at these domains' })
      .click();
    expect(await page.locator('[aria-label="remove a@x.dev"]').count()).toBe(0);
    expect(await navAction.isDisabled()).toBe(true);
    expect(
      await page.getByRole('textbox', { name: 'add domain' }).count()
    ).toBe(1);
...
test('root: a teardown failure on turn-off surfaces here even though the toggle already reads off', async () => {
  await withBoard(async page => {
    ...
    expect(
      await page.locator('[aria-label="require google sign-in"]').count()
    ).toBe(1); // relabeled, now off
    expect(await accessNav(page, 'who').count()).toBe(0);
  });
});
