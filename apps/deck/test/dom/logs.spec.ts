...
async function openLogs(page: Page, name: string): Promise<void> {
  ...
  await page
    ...
    .filter({
      has: page.locator('[data-part="listgroup-label"]', { hasText: 'logs' }),
    })
    .locator('button')
    .click();
...
test('logs screen shows a richer stderr tail landed by the next poll, newest at the bottom', async () => {
  await withBoard(async page => {
    await openLogs(page, 'ledger');
    ...
    await waitUntil(async () =>
      ((await box.textContent()) ?? '').includes('respawning')
    );
    expect((await box.textContent())?.trim()).toBe(
      'bun: error: connect ECONNREFUSED\nexited with code 1\nlaunchd: respawning in 10s'
    );
...
test('logs screen live-updates when a later poll returns a new stderr line', async () => {
  await withBoard(async page => {
    await openLogs(page, 'ledger');
    const box = page.locator('.drawer-logbox');
    expect((await box.textContent())?.trim()).toBe(
