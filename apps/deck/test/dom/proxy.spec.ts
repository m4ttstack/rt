  ...
  await withBoard(async page => {
    ...
    await page.waitForFunction(() =>
      [...document.querySelectorAll('button')].some(b =>
        b.textContent?.includes('restarting…')
      )
    );
...
test('not-authorized restart shows the one-time-setup Alert with the install command in the command block', async () => {
  await withBoard(async page => {
    ...
    expect(
      await alert.locator('[data-part="alert-command"]').textContent()
    ).toBe('sudo portless install-rule --allow-restart');
  });
});

test('successful reload waits out one /healthz drop then shows the ok notice', async () => {
  await withBoard(async page => {
    let healthzCalls = 0;
    ...
    await page.waitForFunction(
      () => {
        const el = document.querySelector('[data-part="alert"]');
        return (
          !!el && (el.textContent ?? '').includes('portless proxy restarted')
        );
      },
      undefined,
      { timeout: 8000 }
...
test('the explicit ok notice outranks the automatic stale-proxy banner until its hold expires', async () => {
  await withBoard(async page => {
    let statusCalls = 0;
    ...
    await page.route('**/healthz', async route => {
      healthzCalls++;
      await route.fulfill({
        status: healthzCalls === 1 ? 503 : 200,
        body: healthzCalls === 1 ? 'down' : 'ok',
      });
    });
