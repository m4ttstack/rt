...
function remoteToggle(page: Page) {
  return page.locator('[data-part="listgroup-toggle"]').filter({
...
test('a live-remote row shows a public: railway marker and a Push to Railway button', async () => {
  await withBoard(
    async page => {
      ...
      expect(
        await row
          .locator('[data-part="chip"]', { hasText: 'public: railway' })
          .count()
      ).toBe(1);
      ...
      expect(
        await plain.locator('[aria-label="push atlas to Railway"]').count()
      ).toBe(0);
    },
...
test("a non-remote row's drawer renders an enabled Remote toggle, off", async () => {
  await withBoard(
    async page => {
      ...
      expect(
        await page
          .locator('[data-part="listgroup-fact"]', { hasText: 'status' })
          .count()
      ).toBe(0);
      expect(
        await page
...
test("a live row's drawer reflects remote.status and its Push action POSTs to /push", async () => {
  await withBoard(
    async page => {
      ...
      const toggle = remoteToggle(page);
      expect(
        await toggle.locator('[data-part="switch-control"]').isChecked()
      ).toBe(true);
      const status = page.locator('[data-part="listgroup-fact"]').filter({
        has: page.locator('[data-part="listgroup-label"]', {
          hasText: 'status',
        }),
      });
      expect(
        await status.locator('[data-part="listgroup-value"]').textContent()
      ).toBe('live');

      const push = page.locator('[data-part="listgroup-action"] button', {
        hasText: 'Push to Railway',
      });
      expect(await push.count()).toBe(1);
      expect(await push.isDisabled()).toBe(false);
...
test('a password-only row (no sign-in gate) has its Remote toggle disabled with a tooltip', async () => {
  await withBoard(
    async page => {
      ...
      const control = remoteToggle(page).locator(
