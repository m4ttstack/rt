  ...
  test('register -> serve -> override -> unregister with real launchd', async () => {
    ...
    try {
      ...
      for (let i = 0; i < 30 && !up; i++) {
        ...
        up = await fetch(`http://127.0.0.1:${record.port}/`)
          .then(r => r.ok)
          .catch(() => false);
      }
      expect(up).toBe(true);
      ...
      writeFileSync(
        ...
        JSON.stringify([
          { hostname: `${NAME}.localhost`, port: record.port, pid: 0 },
        ])
      );
      const ov = await fetch(
        `http://127.0.0.1:${PORT}/api/v1/apps/${NAME}/override`,
        {
          method: 'PUT',
          headers: { 'content-type': 'application/json' },
      ...
      const routes = JSON.parse(
        await Bun.file(process.env.LOCAL_APPS_ROUTES_PATH!).text()
      );
      expect(routes[0].port).toBe(19555);

      // unregister: service gone from launchd and disk
      const del = await fetch(`http://127.0.0.1:${PORT}/api/v1/apps/${NAME}`, {
        method: 'DELETE',
      });
      ...
      rmSync(join(homedir(), 'Library', 'LaunchAgents', `${LABEL}.plist`), {
        force: true,
      });
      server.stop(true);
