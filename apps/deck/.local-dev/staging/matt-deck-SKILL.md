...
and routes on this machine). The command is `deck`, not `local` (`local` is a
shell reserved word). Never write a plist or call `portless alias` /
`launchctl load` directly - Deck is the sole writer; hand-written artifacts
are exactly what `deck migrate` exists to clean up.
