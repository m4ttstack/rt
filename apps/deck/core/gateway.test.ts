...
import { appFromHost, decide, publicPort, safeNext } from './gateway.ts';
...
test('appFromHost strips port and known suffixes', () => {
  ...
  expect(appFromHost('nihongo.example.dev:7950', 'example.dev')).toBe(
    'nihongo'
  );
...
test('appFromHost strips any configured TLD', () => {
  expect(appFromHost('foo.mattstack', null, ['localhost', 'mattstack'])).toBe(
    'foo'
  );
  expect(appFromHost('foo.localhost', null, ['localhost', 'mattstack'])).toBe(
    'foo'
  );
});
...
test('no route stays undefined, so the gateway still reports no-route', () => {
  ...
  expect(
    decide({
      app: 'x',
      port: publicPort(undefined, undefined),
      published: true,
      passwordVersion: 0,
      secret: SECRET,
    }).kind
