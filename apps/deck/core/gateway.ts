...
export function appFromHost(
  ...
  if (publicDomain && h.endsWith(`.${publicDomain}`))
    return h.slice(0, -(publicDomain.length + 1));
...
async function proxyWebSocket(
  ...
  const headers = negotiated
    ? { 'sec-websocket-protocol': negotiated }
    : undefined;
  ...
  const server = Bun.serve<WsProxyData>({
    ...
    async fetch(req): Promise<Response | undefined> {
      ...
      const port = publicPort(
        routes.get(app),
        settings.override,
        settings.publicFollowsOverride
      );
      ...
      if (req.method === 'POST' && url.pathname === '/__auth') {
        ...
        if (await Bun.password.verify(password, settings.passwordHash)) {
          ...
          return new Response(null, {
            status: 303,
            headers: { location: next, 'set-cookie': cookieHeader(token) },
          });
        }
        recordFailure(key);
      ...
      switch (d.kind) {
        ...
        case 'needs-login':
          return html(
            pageLogin(d.app, { next: url.pathname + url.search }),
            401
          );
        case 'proxy':
          // Upgrades cannot travel through fetch(), so they branch off here.
          if (isWebSocketUpgrade(req))
            return await proxyWebSocket(server, req, url, d.port, d.app);
          try {
            return await proxyTo(d.port, req, url);
          } catch {
            return html(pageOffline(d.app), 502);
          }
      }
    },
  });
