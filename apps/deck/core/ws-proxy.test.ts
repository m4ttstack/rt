...
test('a real upgrade request is recognised', () => {
  expect(
    isWebSocketUpgrade(req({ upgrade: 'websocket', connection: 'Upgrade' }))
  ).toBe(true);
});
...
test('Connection is a list in practice, so a token match is required', () => {
  ...
  expect(
    isWebSocketUpgrade(
      req({ upgrade: 'websocket', connection: 'keep-alive, Upgrade' })
    )
  ).toBe(true);
});
...
test('Upgrade without a Connection token is not an upgrade', () => {
  expect(
    isWebSocketUpgrade(req({ upgrade: 'websocket', connection: 'keep-alive' }))
  ).toBe(false);
});
...
test('a subprotocol preference list keeps its order', () => {
  expect(requestedProtocols('vite-hmr, graphql-ws')).toEqual([
    'vite-hmr',
    'graphql-ws',
  ]);
});
...
test('upstream url keeps the path and query the client asked for', () => {
  ...
  expect(upstreamUrl(4101, '/hmr', '?token=abc')).toBe(
    'ws://127.0.0.1:4101/hmr?token=abc'
  );
});
