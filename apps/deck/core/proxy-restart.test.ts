...
test('the install command validates with visudo before activating the rule', () => {
  ...
  expect(cmd.indexOf(`visudo -c -f ${tmp}`)).toBeGreaterThan(
    cmd.indexOf(`> ${tmp}`)
  );
...
test("isNotAuthorized recognizes sudo's refusal messages", () => {
  ...
  expect(
    isNotAuthorized('sudo: a terminal is required to read the password')
  ).toBe(true);
  expect(
    isNotAuthorized(
      "Sorry, user matt is not allowed to execute '/bin/launchctl' as root"
    )
  ).toBe(true);
