...
writeFileSync(
  ROUTES,
  JSON.stringify([{ hostname: `${APP}.localhost`, port: dev.port, pid: 0 }])
);
...
test('a websocket upgrade reaches the dev server and frames flow both ways', async () => {
  ...
  const done = new Promise<void>((resolve, reject) => {
    const timer = setTimeout(
      () => reject(new Error('no frames within 5s')),
      5000
    );
    ws.onmessage = e => {
...
test('frames sent before the client socket opens are queued, not dropped', async () => {
  ...
  const first = await new Promise<string>((resolve, reject) => {
    const timer = setTimeout(
      () => reject(new Error('greeting never arrived')),
      5000
    );
    ws.onmessage = e => {
      clearTimeout(timer);
