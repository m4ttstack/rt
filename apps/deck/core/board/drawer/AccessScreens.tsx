...
export const buildAccessRoot: ScreenBuilder = (
  row,
  nav,
  board
): DrawerScreen => {
...
export const buildAccessPassword: ScreenBuilder = (
  row,
  nav,
  board
): DrawerScreen => {
      ...
      <div className="drawer-groups">
        <ListGroup footer="replaces the current password immediately for new visitors">
          <ListGroup.Input
            ...
            value={value}
            onChange={ev =>
              board.updateAccessModal({ password: ev.target.value })
            }
...
export const buildAccessWho: ScreenBuilder = (
  row,
  nav,
  board
): DrawerScreen => {
    ...
    content: (
      <div className="drawer-groups">
        <div role="radiogroup" aria-label="who can sign in">
          <ListGroup>
            <ModeRow
              label="anyone at these domains"
              selected={mode === 'domains'}
              onSelect={() => selectMode('domains')}
            />
            <ModeRow
              label="these people"
              selected={mode === 'emails'}
              onSelect={() => selectMode('emails')}
            />
          </ListGroup>
        </div>
