...
export function overridesToReassert(
  ...
  for (const [app, ov] of Object.entries(overrides)) {
    const route = routes.find(
      r => r.hostname.replace(/\.localhost$/, '') === app
    );
    if (route && route.port !== ov.devPort)
      out.push({ hostname: app, devPort: ov.devPort });
  }
  return out;
}
...
function hasPendingRemoteWork(): boolean {
  return listRecords().some(
    r => r.remote?.status === 'deploying' || r.remote?.status === 'verifying'
  );
}
...
async function reconcileRemoteTick(): Promise<void> {
  ...
  try {
    ...
    if (
      ...
      !secrets.cfApiToken ||
      !secrets.cfZoneId ||
      !secrets.railwayApiToken ||
      !secrets.railwayToken ||
      !rc
    )
      return;
    const railway = new RailwayCli({
      apiToken: secrets.railwayApiToken,
      projectToken: secrets.railwayToken,
      projectId: rc.projectId,
      environmentId: rc.environmentId,
    });
    const dns = new CfDnsApi({
      zoneId: secrets.cfZoneId,
      token: secrets.cfDnsToken ?? secrets.cfApiToken,
    });
