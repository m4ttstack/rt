...
const { getPlatformSettings, reloadPlatformSettings } =
...
beforeEach(() => {
  ...
  writeFileSync(
    join(process.env.LOCAL_AGENTS_DIR!, 'com.matthewgoodwin.boxscore.plist'),
    PLIST('com.matthewgoodwin.boxscore', 11005)
  );
  writeFileSync(
...
test('adopts a route with no service as an external record', async () => {
  ...
  expect(getRecord('mattari')!).toMatchObject({
    kind: 'external',
    port: 4101,
    grandfathered: true,
  });
...
test('does not adopt two routes at the same never-before-claimed port as separate records within one run', async () => {
  ...
  writeFileSync(
    join(process.env.LOCAL_AGENTS_DIR!, 'com.matthewgoodwin.widgets.plist'),
    PLIST('com.matthewgoodwin.widgets', 9999)
  );
  writeFileSync(
    process.env.LOCAL_APPS_ROUTES_PATH!,
    JSON.stringify([
  ...
  expect(getRecord('widgets')).toMatchObject({
    label: 'com.matthewgoodwin.widgets',
    managedBy: 'user',
  });
  expect(getRecord('widgets.otherapp')).toBeUndefined();

  const { listRecords } = await import('./records.ts');
  const sharingLabel = listRecords().filter(
    r => r.label === 'com.matthewgoodwin.widgets'
  );
...
test('writes NOTHING to the plist file or routes.json - adopts in place, never rewrites', async () => {
  const plistPath = join(
    process.env.LOCAL_AGENTS_DIR!,
    'com.matthewgoodwin.boxscore.plist'
  );
