...
test('null when displayName is missing', () => {
  expect(
    readManifest(
      repo({ 'mattstack.json': JSON.stringify({ icon: './i.svg' }) })
    )
  ).toBeNull();
});
...
test('null when icon is missing', () => {
  expect(
    readManifest(
      repo({ 'mattstack.json': JSON.stringify({ displayName: 'X' }) })
    )
  ).toBeNull();
});
...
test('description is optional', () => {
  const dir = repo({
    'mattstack.json': JSON.stringify({ displayName: 'X', icon: './i.svg' }),
  });
...
test('re-ingest overwrites identity: a second ingestManifest call picks up a changed displayName', async () => {
  ...
  writeFileSync(
    ...
    JSON.stringify({ displayName: 'Two', icon: './icon.svg' })
  );
  ingestManifest('resync');
...
test('removeIcon deletes the stored file', async () => {
  ...
  reloadRegistry();
  const appDir = repo({
    'mattstack.json': JSON.stringify({ displayName: 'C', icon: './i.svg' }),
    'i.svg': SVG,
  });
  putRecord({
    name: 'c',
