...
describe('readLinkedManifest', () => {
  ...
  test('broken when the manifest is missing or unparseable', () => {
    ...
    expect(
      readLinkedManifest(rec({ dev: { workingDirectory: dir } })).state
    ).toBe('broken');
  ...
  test('linked returns the parsed manifest and dir', () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({ name: 'chat', dev: { start: 'bun x' } })
    );
...
const CHAT_DEV = {
  name: 'chat',
  dev: { start: 'bun src/server/index.ts', build: 'bun run build' },
};
...
describe('serveShape matrix', () => {
  test('user row: record shape, any mode', () => {
    ...
    putRecord(r);
    expect(serveShape(r, { devMode: () => true })).toEqual({
      command: ['node', 's.js'],
      cwd: '/x',
    });
  });
  ...
  test('legacy stored command with a bundle installed: serves the derived bundle, still flags the legacy command', () => {
    ...
    const r = rec({
      command: ['bun', 'src/server.ts'],
      workingDirectory: '/data/chat',
    });
    putRecord(r);
    expect(
      serveShape(r, { devMode: () => false, helpersDir: helpers })
    ).toEqual({
      command: [join(helpers, 'chat')],
      cwd: dataDir('chat'),
    });
  ...
  test('dev + broken link + bundle: bundle with loud issue; issue clears on next clean resolve', () => {
    ...
    putRecord(r);
    expect(
      serveShape(r, { devMode: () => true, helpersDir: helpers })?.command
    ).toEqual([join(helpers, 'chat')]);
  ...
  test('linked fresh-install row: a stored command inside the helpers dir is the bundle shape, args kept', () => {
    ...
    expect(
      serveShape(r, { devMode: () => false, helpersDir: helpers })
    ).toEqual({ command: [bin, 'board'], cwd: '/data/chat' });
    expect(
      serveShape(r, { devMode: () => true, helpersDir: helpers })?.cwd
    ).toBe(dir);
  });
  ...
  test('linked row: a stored command OUTSIDE the helpers dir is never the bundle shape', () => {
    ...
    expect(serveShape(r, { devMode: () => true, helpersDir: null })?.cwd).toBe(
      dir
    );
