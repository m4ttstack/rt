...
const { getRecord, putRecord, deleteRecord, reloadRegistry } =
  await import('./records.ts');
const { FakeServiceManager } = await import('../services/fake.ts');
const { PLATFORM_LABEL, LABEL_PREFIX, LEGACY_PLATFORM_LABEL_PREFIX } =
...
const healthyAt =
  (port: number): ((p: number) => Promise<Health>) =>
  async p =>
    p === port
      ? { ok: true, status: 200, ms: 1 }
      : { ok: false, status: null, ms: null };
const alwaysUnhealthy: (p: number) => Promise<Health> = async () => ({
  ok: false,
  status: 503,
  ms: 1,
});
const alwaysHealthy: (p: number) => Promise<Health> = async () => ({
  ok: true,
  status: 200,
  ms: 1,
});
...
test('a record labeled under the pre-rename com.mattstack.local. prefix re-converts to com.mattstack.deck.', async () => {
  putRecord(
    legacyRecord('foo', 11010, { label: `${LEGACY_PLATFORM_LABEL_PREFIX}foo` })
  );
  const manager = new FakeServiceManager();
  ...
  expect(manager.installed.has(newLabel)).toBe(true);
  expect(manager.installed.has(`${LEGACY_PLATFORM_LABEL_PREFIX}foo`)).toBe(
    false
  );
...
test('legacy plist file is removed on success and restored on rollback (real LaunchdManager, faked exec)', async () => {
  ...
  try {
    ...
    const oldPlistPath = join(
      agentsDir,
      `${DEFAULT_LEGACY_PREFIX}boxscore.plist`
    );
    const newPlistPath = join(agentsDir, `${LABEL_PREFIX}boxscore.plist`);

    const okResult = await convert({
      manager,
      healthCheck: alwaysHealthy,
      waitMs: 30,
      intervalMs: 5,
    ...
    expect(readFileSync(newPlistPath, 'utf8')).toContain(
      '<string>11005</string>'
    );
    ...
    putRecord(legacyRecord('boxscore', 11005));
    const failResult = await convert({
      manager,
      healthCheck: alwaysUnhealthy,
      waitMs: 30,
      intervalMs: 5,
