...
test('skips gitq-shaped (no includeInBundle) and fresh-install (no manifest) rows untouched', () => {
  ...
  putRecord(
    row({
      name: 'gitq',
      command: ['/b/gitq', 'board'],
      workingDirectory: gitqDir,
    })
  );
  putRecord(
    row({
      name: 'console',
      command: ['/b/console'],
      workingDirectory: '/home/.mattstack/console',
    })
  );
  migrateManagedDevShape();
...
test('platform row: sourceDirectory moves to dev.workingDirectory, command survives', () => {
  putRecord(
    row({
      name: 'deck',
      managedBy: 'deck',
      command: ['/app/deck', 'serve'],
      sourceDirectory: '/repos/deck',
      commands: { deploy: 'd' },
    })
  );
  migrateManagedDevShape();
...
test('uptime guard throws on a slim write without a dev link', () => {
  expect(() => assertSlimRowKeepsAFallback('chat', row({}))).toThrow(
    /refusing to slim/
  );
});
