...
test('user caller may remove a user record', () => {
  expect(
    authorizeStructural({ name: 'a', managedBy: 'user' }, 'user', false)
  ).toEqual({ ok: true });
});

test("user caller on an rt-managed record gets the spec's exact 409", () => {
  const v = authorizeStructural(
    { name: 'gitq', managedBy: 'rt' },
    'user',
    false
  );
...
test('the registrar itself passes; force is the escape hatch for everyone', () => {
  expect(
    authorizeStructural({ name: 'gitq', managedBy: 'rt' }, 'rt', false).ok
  ).toBe(true);
  expect(
    authorizeStructural({ name: 'gitq', managedBy: 'rt' }, 'user', true).ok
  ).toBe(true);
});

test("Deck's own record is 409-gated to `deck uninstall`", () => {
  const v = authorizeStructural(
    { name: 'deck', managedBy: 'deck' },
    'user',
    false
  );
  expect(v.ok).toBe(false);
  if (!v.ok)
    expect(v.body.message).toBe('This is Deck itself: `deck uninstall`');
});
