...
const { LEGACY_PLATFORM_LABEL, PLATFORM_NAME } =
...
test('bootstrap: agent + aliases first, then the record catches up as managedBy deck', async () => {
  ...
  const result = await bootstrapSelf(
    { manager, edge },
    {
...
test("the platform's own plist carries a composed PATH, not the installing shell's", async () => {
  ...
  process.env.PATH =
    '/fake/installer/bin:/home/t/.local/state/fnm_multishells/1_2/bin';
  try {
    await bootstrapSelf(
      { manager, edge },
      {
...
test('bootstrap is idempotent: a second run (reinstall/upgrade/retry) does not throw', async () => {
  ...
  expect(manager.installed.get('com.mattstack.deck')!.environment.PORT).toBe(
    String(first.port)
  );
...
test('checkout mode: dev is not set when manifest name does not match PLATFORM_NAME', async () => {
  ...
  writeFileSync(
    ...
    JSON.stringify({
      name: 'other-app',
      commands: { deploy: 'bun run deploy' },
    })
  );
