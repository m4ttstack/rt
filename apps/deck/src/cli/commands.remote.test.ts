...
const { FakeRailwayDriver, FakeCfDns } =
  await import('../../test/fixture/remote.ts');
const { reloadRegistry, putRecord, getRecord } =
  await import('../registry/records.ts');
const { reloadPlatformSettings, updatePlatformSettings } =
...
beforeAll(() => {
  ...
  server = startApi({
    ...
    deckSecrets: {
      ...
      post: async () => ({
        ok: true,
        data: { railwayToken: secretsConfig.railwayToken },
      }),
    },
  });
  writeApiInfo(PORT);
...
test('deck push site prints the pushed sha', async () => {
  ...
  putRecord({
    ...
    remote: {
      target: 'railway',
      serviceId: 'svc_1',
      customDomain: 'pushme.m4tthew.dev',
      status: 'live',
    },
  });
