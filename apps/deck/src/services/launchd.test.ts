...
test('install writes the plist then loads it; uninstall unloads then removes', async () => {
  const calls: string[][] = [];
  const mgr = new LaunchdManager(async argv => {
    calls.push(argv);
    return 0;
  });
  const spec = {
...
test('uninstall treats a bootout of a non-loaded label (nonzero unload exit) as success', async () => {
  ...
  const failingUnload = new LaunchdManager(async argv =>
    argv[1] === 'unload' ? 3 : 0
  );
...
test('readInstalledProgramArguments round-trips renderPlist, escapes included', async () => {
  ...
  try {
    ...
    expect(readInstalledProgramArguments('com.mattstack.deck.chat')).toEqual(
      spec.programArguments
    );
    expect(
      readInstalledProgramArguments('com.mattstack.deck.ghost')
    ).toBeNull();
  } finally {
...
test("readInstalledWorkingDirectory round-trips renderPlist's WorkingDirectory, escapes included, and returns null when absent", async () => {
  ...
  try {
    ...
    await manager.install(spec);
    expect(readInstalledWorkingDirectory('com.mattstack.deck.chat')).toBe(
      spec.workingDirectory
    );
    expect(
      readInstalledWorkingDirectory('com.mattstack.deck.ghost')
    ).toBeNull();
  } finally {
    rmSync(testDir, { recursive: true, force: true });
    process.env.LOCAL_AGENTS_DIR = dir;
...
test("readInstalledEnvironment round-trips renderPlist's EnvironmentVariables, PATH default and escapes included, and returns null when absent", async () => {
  const testDir = mkdtempSync(join(tmpdir(), 'agents-'));
  process.env.LOCAL_AGENTS_DIR = testDir;
  try {
    const manager = new LaunchdManager(async () => 0);
    await manager.install({
      ...
      programArguments: ['/usr/bin/env'],
      workingDirectory: '/tmp',
      environment: { PORT: '11002', ODD: 'a&b<c>d' },
      stdoutPath: '/tmp/o',
      stderrPath: '/tmp/e',
    });
    const env = readInstalledEnvironment('com.mattstack.deck.chat');
    expect(env).toMatchObject({ PORT: '11002', ODD: 'a&b<c>d' });
    expect(env?.PATH).toBeTruthy();
    expect(readInstalledEnvironment('com.mattstack.deck.ghost')).toBeNull();
  } finally {
    rmSync(testDir, { recursive: true, force: true });
    process.env.LOCAL_AGENTS_DIR = dir;
  }
