...
test('end-to-end: adopt a fixture app, then list/serve/CORS/resync it through the real API', async () => {
  ...
  const list = await fetch(`http://127.0.0.1:${PORT}/api/apps`, {
    headers: apiHeaders(),
  });
  ...
  const icon = await fetch(`http://127.0.0.1:${PORT}/api/apps/chat/icon`, {
    headers: apiHeaders(),
  });
  ...
  const readopt = await fetch(
    `http://127.0.0.1:${PORT}/api/v1/apps/chat/adopt`,
    {
      method: 'POST',
      headers: apiHeaders({ 'content-type': 'application/json' }),
      body: JSON.stringify({ managedBy: 'rt' }),
  ...
  const relisted = await fetch(`http://127.0.0.1:${PORT}/api/apps`, {
    headers: apiHeaders(),
  });
