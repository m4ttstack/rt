...
test('railway defaults to null and round-trips (unowned: via platform.json)', () => {
  ...
  reloadPlatformSettings(() => ({ value: undefined }));
  expect(getPlatformSettings().railway).toEqual({
    projectId: 'p1',
    environmentId: 'e1',
  });
});
...
test('railway is store-migrated: the deck.platform store wins over the file', () => {
  ...
  updatePlatformSettings(
    { railway: { projectId: 'file-p', environmentId: 'file-e' } },
    () => ({ value: undefined })
  );
  reloadPlatformSettings(() => ({
    value: { railway: { projectId: 'store-p', environmentId: 'store-e' } },
  }));
  expect(getPlatformSettings().railway).toEqual({
    projectId: 'store-p',
    environmentId: 'store-e',
  });
});
