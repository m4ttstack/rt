...
process.env.LOCAL_AGENTS_DIR = mkdtempSync(
  join(tmpdir(), 'local-status-tunnel-agents-')
);
...
test('the deck tunnel row carries edge health when a domain is bound', async () => {
  updatePlatformSettings({
    publicDomain: 'e.dev',
    tunnel: { name: 'deck-edge-x-abc123', uuid: 'u' },
  });
  const s = await buildStatus({
    ...
    autoHeal: null,
    readyFetch: (async () =>
      Response.json({ status: 200, readyConnections: 3 })) as any,
  });
