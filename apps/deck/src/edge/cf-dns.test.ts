...
test('proxied cname + txt write and delete leave no host records', async () => {
  ...
  await dns.writeProxiedCname(
    'site.m4tthew.dev',
    'site.m4tthew.dev.up.railway.app'
  );
  expect(dns.cname.get('site.m4tthew.dev')).toEqual({
    target: 'site.m4tthew.dev.up.railway.app',
    proxied: true,
  });
...
test('tokenCanEditDns probes a real dns_records read, not just token status', async () => {
  ...
  const accessOnlyDns = new CfDnsApi({
    zoneId: 'z1',
    token: 'access-tok',
    fetchImpl: accessOnlyFetch,
  });
  ...
  const zoneDnsFetch = (async (url: string | URL | Request) => {
    ...
    return Response.json({
      success: true,
      result: { id: 'tok-2', status: 'active' },
    });
  }) as typeof fetch;
  const zoneDnsDns = new CfDnsApi({
    zoneId: 'z1',
    token: 'zone-dns-tok',
    fetchImpl: zoneDnsFetch,
  });
...
test('writeProxiedCname updates an existing record instead of creating a duplicate', async () => {
  ...
  const fetchImpl = (async (
    ...
    if (method === 'GET')
      return Response.json({ success: true, result: [existing] });
    if (method === 'POST')
      return Response.json({
        success: false,
        errors: [{ code: 81053, message: 'already exists' }],
      });
    return Response.json({
      success: true,
      result: { ...existing, content: body.content },
    });
  }) as typeof fetch;
...
test("cnameTarget reads the existing record's content + proxied state, null when absent", async () => {
  ...
  const fetchImpl = (async (url: string | URL | Request) =>
    Response.json({
      success: true,
      result:
        new URL(String(url)).searchParams.get('name') === '*.example.dev'
          ? [
              {
                id: 'r',
                type: 'CNAME',
                name: '*.example.dev',
                content: 'u.cfargotunnel.com',
                proxied: true,
              },
            ]
          : [],
  ...
  const dns = new CfDnsApi({ zoneId: 'z1', token: 't', fetchImpl });
  expect(await dns.cnameTarget('*.example.dev')).toEqual({
    target: 'u.cfargotunnel.com',
    proxied: true,
  });
...
test('FakeCfDns mirrors upsert + cnameTarget', async () => {
  ...
  expect(await dns.cnameTarget('*.e.dev')).toEqual({
    target: 'b.cfargotunnel.com',
    proxied: true,
  });
