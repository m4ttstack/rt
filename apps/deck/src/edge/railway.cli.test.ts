...
function fakeGraphql(handlers: Record<string, (vars: any) => any> = {}) {
  ...
  const doFetch = (async (_url: string, init: any) => {
    ...
    const op = /mutation\s*\(|query\s*\(/.test(body.query)
      ? (body.query.match(
          /\b(serviceCreate|serviceInstanceUpdate|variableCollectionUpsert|serviceDelete|project)\b/
        )?.[1] ?? 'unknown')
      : 'unknown';
...
test('ensureService: creates when absent, reuses when present', async () => {
  const g1 = fakeGraphql({
    project: () => ({ project: { services: { edges: [] } } }),
    serviceCreate: () => ({ serviceCreate: { id: 'svc-new' } }),
  });
  const rw1 = new RailwayCli({ ...CFG, doFetch: g1.doFetch });
  expect(
    await rw1.ensureService('deck-x', { projectId: 'p1', environmentId: 'e1' })
  ).toEqual({ serviceId: 'svc-new', created: true });
  expect(g1.calls.map(c => c.op)).toEqual(['project', 'serviceCreate']);

  const g2 = fakeGraphql({
    project: () => ({
      project: {
        services: { edges: [{ node: { id: 'svc-old', name: 'deck-x' } }] },
      },
    }),
  });
  const rw2 = new RailwayCli({ ...CFG, doFetch: g2.doFetch });
  expect(
    await rw2.ensureService('deck-x', { projectId: 'p1', environmentId: 'e1' })
  ).toEqual({ serviceId: 'svc-old', created: false });
...
test('configureService: sets build/start then upserts variables incl PORT', async () => {
  ...
  await rw.configureService('svc1', {
    buildCommand: 'b',
    startCommand: 's',
    port: 11010,
    variables: { API: 'x' },
  });
  expect(g.calls.map(c => c.op)).toEqual([
...
test('deleteService issues serviceDelete with the account token', async () => {
  ...
  let authHeader = '';
  const doFetch = (async (_u: string, init: any) => {
    authHeader = init.headers.authorization;
    return g.doFetch(_u, init);
  }) as any;
...
function fakeExec(byCmd: (argv: string[]) => { code: number; stdout: string }) {
  const calls: string[][] = [];
  const exec = async (argv: string[], _o: any) => {
    calls.push(argv);
    return byCmd(argv);
  };
  return { exec, calls };
...
const STATUS_JSON = JSON.stringify({
  domain: {
    ...
    verification: {
      verified: true,
      dnsHost: '_railway-verify.app',
      token: 'railway-verify=hex',
    },
  },
});
...
test('ensureCustomDomain parses CNAME (create) + TXT (status)', async () => {
  const { exec, calls } = fakeExec(argv =>
    argv.includes('status')
      ? { code: 0, stdout: STATUS_JSON }
      : { code: 0, stdout: CREATE_JSON }
  );
  ...
  expect(d).toEqual({
    ...
    txtName: '_railway-verify.app',
    txtValue: 'railway-verify=hex',
    created: true,
  });
...
test('domainStatus reads verification.verified', async () => {
  ...
  const rw = new RailwayCli({ ...CFG, exec });
  expect(await rw.domainStatus('svc1', 'app.m4tthew.dev')).toEqual({
    verified: true,
    proxyDetected: true,
  });
});
...
test('up returns ok:true on exit 0, ok:false on non-zero (no throw)', async () => {
  const ok = new RailwayCli({
    ...CFG,
    exec: fakeExec(() => ({ code: 0, stdout: 'built' })).exec,
  });
  expect((await ok.up('svc1', { cwd: '/tmp/a', token: 'proj-tok' })).ok).toBe(
    true
  );
  const bad = new RailwayCli({
    ...CFG,
    exec: fakeExec(() => ({ code: 1, stdout: 'boom' })).exec,
  });
  expect((await bad.up('svc1', { cwd: '/tmp/a', token: 'proj-tok' })).ok).toBe(
    false
  );
});
