...
test('ensureService is idempotent by name (reuse, never re-create)', async () => {
  ...
  const a = await rw.ensureService('deck-site', {
    projectId: 'p',
    environmentId: 'e',
  });
  const b = await rw.ensureService('deck-site', {
    projectId: 'p',
    environmentId: 'e',
  });
...
test('domainStatus is scriptable and ensureCustomDomain returns TXT + CNAME target', async () => {
  const rw = new FakeRailwayDriver();
  const { serviceId } = await rw.ensureService('deck-site', {
    projectId: 'p',
    environmentId: 'e',
  });
  ...
  expect(await rw.domainStatus(serviceId, 'site.m4tthew.dev')).toEqual({
    verified: true,
    proxyDetected: true,
  });
});
