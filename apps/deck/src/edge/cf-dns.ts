...
export interface CfDns {
  ...
  cnameTarget(
    host: string
  ): Promise<{ target: string; proxied: boolean } | null>;
...
export class CfDnsApi implements CfDns {
  ...
  private async req<T>(
    ...
    if (!data.success) {
      throw new Error(
        `Cloudflare API ${method} ${url} failed: ${JSON.stringify(data).slice(0, 300)}`
      );
    }
    return data.result;
  }
  ...
  private async listRecords(
    ...
    return this.req<CfDnsRecord[]>(
      'GET',
      `${BASE}/zones/${this.zoneId}/dns_records?${q.toString()}`
    );
  }

  async zoneSslMode(): Promise<ZoneSslMode> {
    const result = await this.req<{ value: ZoneSslMode }>(
      'GET',
      `${BASE}/zones/${this.zoneId}/settings/ssl`
    );
    return result.value;
  }
  ...
  async tokenCanEditDns(): Promise<boolean> {
    ...
    try {
      await this.req(
        'GET',
        `${BASE}/zones/${this.zoneId}/dns_records?per_page=1`
      );
      return true;
    } catch {
      return false;
  ...
  async deleteTxt(name: string): Promise<void> {
    ...
    for (const record of records) {
      await this.req(
        'DELETE',
        `${BASE}/zones/${this.zoneId}/dns_records/${record.id}`
      );
    }
  }
  ...
  async writeProxiedCname(host: string, target: string): Promise<void> {
    ...
    if (existing) {
      await this.req(
        'PATCH',
        `${BASE}/zones/${this.zoneId}/dns_records/${existing.id}`,
        payload
      );
      return;
    }
  ...
  async cnameTarget(
    ...
    const existing = (await this.listRecords(host, 'CNAME'))[0];
    return existing
      ? { target: existing.content, proxied: !!existing.proxied }
      : null;
  }
...
export async function resolveCfDns(
  deps?: RtSecretsDeps
): Promise<CfDns | null> {
  const s = await readDeckSecrets(deps);
  const token = s.ok ? (s.cfDnsToken ?? s.cfApiToken) : undefined;
  return s.ok && s.cfZoneId && token
    ? new CfDnsApi({ zoneId: s.cfZoneId, token })
    : null;
}
