...
export class CloudflaredCli implements TunnelDriver {
  ...
  private async run(args: string[], what: string): Promise<string> {
    ...
    if (code !== 0)
      throw new Error(
        `cloudflared tunnel ${what} failed: ${(stdout + stderr).slice(0, 300)}`
      );
  ...
  > {
    ...
    if (code !== 0)
      throw new Error(
        `cloudflared tunnel list failed: ${(stdout + stderr).slice(0, 300)}`
      );
    const rows = JSON.parse(stdout) as ListedTunnel[];
    return rows.map(r => ({
      name: r.name,
      uuid: r.id,
      connections: r.connections?.length ?? 0,
    }));
  }
  ...
  async info(name: string): Promise<{ connectors: number }> {
    ...
    if (code !== 0)
      throw new Error(
        `cloudflared tunnel info ${name} failed: ${(stdout + stderr).slice(0, 300)}`
      );
...
export class FakeTunnelDriver implements TunnelDriver {
  ...
  async list() {
    ...
    return [...this.tunnels].map(([name, uuid]) => ({
      name,
      uuid,
      connections: 0,
    }));
