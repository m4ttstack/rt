...
const { getPlatformSettings, reloadPlatformSettings, updatePlatformSettings } =
...
test('identity is recorded before DNS or the service are touched', async () => {
  ...
  dns.writeProxiedCname = async () => {
    throw new Error('cf down');
  };
  await expect(
    bindDomain('example.dev', { tunnel, manager, dns }, opts())
  ).rejects.toThrow('cf down');
  expect(getPlatformSettings().tunnel).toEqual({
    name: expect.stringContaining('deck-edge-'),
    uuid: 'fake-uuid-1',
  });
...
test('publicDomain is recorded even when install() throws after DNS is written, so unbind can still clean up the CNAME', async () => {
  ...
  expect(dns.cname.get('*.example.dev')!.target).toBe(
    'fake-uuid-1.cfargotunnel.com'
  );
});
...
test('same-domain re-run reuses the recorded tunnel (creds present) and repairs a drifted config', async () => {
  ...
  expect(readFileSync(join(dir, 'tunnel.yml'), 'utf8')).toContain(
    `tunnel: ${first.uuid}`
  );
});
