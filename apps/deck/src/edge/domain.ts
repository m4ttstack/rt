...
export function defaultCfDir(): string {
  return join(process.env.HOME ?? homedir(), '.cloudflared');
}
export function resolveCloudflared(): string | null {
  return resolveProgram('cloudflared', composeServicePath());
}

// launchd does not search PATH for ProgramArguments[0]; the caller passes an absolute path.
export function tunnelServiceSpec(o: {
  configPath: string;
...
export function expectedTunnelConfig(o: {
  uuid: string;
  domain: string;
  cfDir: string;
  gatewayPort: number;
  ...
  return renderTunnelConfig({
    uuid: o.uuid,
    credentialsFile: credentialsPath(o.cfDir, o.uuid),
    domain: o.domain,
    gatewayPort: o.gatewayPort,
    metricsPort: EDGE_METRICS_PORT,
  });
}
...
export async function bindDomain(
  ...
  if (!DOMAIN_RE.test(domain))
    return { status: 400, body: { error: 'bad domain' } };
  const cfDir = opts.cloudflaredDir ?? defaultCfDir();
  const bin = (opts.resolveBin ?? resolveCloudflared)();
  if (!bin)
    return {
      status: 400,
      body: { error: 'cloudflared-missing', hint: 'brew install cloudflared' },
    };
  ...
  if (!existsSync(join(cfDir, 'cert.pem'))) {
    return {
      status: 428,
      body: {
        error: 'cloudflared-login-required',
  ...
  if (!(await deps.dns.tokenCanEditDns())) {
    return {
      status: 400,
      body: {
        error: 'cf-token-needs-zone-dns',
        hint: 'rt secrets set deck cfDnsToken (Zone.DNS:Edit)',
      },
    };
  }
  ...
  if (current && current !== domain) {
    const remoteApps = listRecords()
      .filter(r => r.remote)
      .map(r => r.name);
    if (remoteApps.length && !opts.force)
      return {
        status: 409,
        body: { error: 'remote-apps-pinned-to-domain', apps: remoteApps },
      };
    const torn = await unbindDomain(deps, {
      force: true,
  ...
  const identity = await resolveTunnel(
    deps.tunnel,
    cfDir,
    opts.random ?? randomSuffix
  );
  ...
  writeTunnelConfig(configPath, {
    uuid: identity.uuid,
    credentialsFile: credentialsPath(cfDir, identity.uuid),
    domain,
    gatewayPort,
    metricsPort: EDGE_METRICS_PORT,
  });
  await deps.dns.writeProxiedCname(
    `*.${domain}`,
    `${identity.uuid}.cfargotunnel.com`
  );
  ...
  await deps.manager.install(
    tunnelServiceSpec({ configPath, cloudflaredBin: bin })
  );
  await deps.manager.kickstart(TUNNEL_LABEL);
  const connectors = await awaitConnector(
    deps.tunnel,
    identity.name,
...
async function resolveTunnel(
  ...
  if (existsSync(credentialsPath(cfDir, listed.uuid)))
    return { name: recorded.name, uuid: listed.uuid };
...
export async function unbindDomain(
  ...
  if (!publicDomain && !tunnel)
    return { status: 200, body: { ok: true, alreadyUnbound: true } };
  ...
  const served = records
    .filter(r => !r.remote && getAppSettings(r.name).published)
    .map(r => r.name);
  if (served.length && !opts.force)
    return {
      status: 409,
      body: { error: 'apps-will-go-offline', apps: served },
    };

  const cfDir = opts.cloudflaredDir ?? defaultCfDir();
  await deps.manager.uninstall(TUNNEL_LABEL);
  if (publicDomain && deps.dns)
    await deps.dns.deleteHostRecords(`*.${publicDomain}`);
  if (tunnel) {
