...
function deps(rw: FakeRailwayDriver, over: Partial<any> = {}) {
  return {
    railway: rw,
    token: 'rw',
    provenance: () => ({ sha: 'abc123', dirty: true }),
...
test('push configures build/start/PORT/env, uploads, records provenance', async () => {
  ...
  expect(getRecord('site')!.remote!.lastPush).toEqual({
    sha: 'abc123',
    dirty: true,
  ...
  expect(rw.upCalls).toEqual([
    { serviceId: 'svc_1', cwd: '/tmp/site', token: 'rw' },
  ]);
});
...
test("a linked/migrated row sends the checkout's dev.start and dev.build, not the record's cleared fields", async () => {
  ...
  writeFileSync(
    ...
    JSON.stringify({
      name: 'site',
      dev: { start: 'bun run dev', build: 'bun run build' },
    })
  );
  putRecord({
    ...rec,
...
test('build failure records a railway SyncIssue and the log', async () => {
  const rw = new FakeRailwayDriver();
  rw.byName.set('deck-site', 'svc_1');
  rw.upResult = { ok: false, log: 'nixpacks: no start' };
  const r = await pushRemote('site', deps(rw));
  expect(r.status).toBe(502);
  expect(getRecord('site')!.issues?.some(i => i.source === 'railway')).toBe(
    true
  );
});
