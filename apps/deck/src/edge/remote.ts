...
export function remoteRefuseChecks(ctx: RefuseCtx): Refusal | null {
  const { record } = ctx;
  if (record.kind !== 'service' || !record.command?.length)
    return refuse(400, 'no start command, cannot push');
  if (!ctx.publicDomain) return refuse(400, 'no-domain-bound');
  if (ctx.publicDomain.split('.').length !== 2)
    return refuse(400, 'public-domain-must-be-first-level');
  ...
  if (ctx.zoneSslMode !== 'full')
    return refuse(400, 'zone-ssl-mode-full-required');
  return null;
}
...
function remoteCommands(
  record: AppRecord
): { start: string; build?: string } | null {
  if (record.dev?.workingDirectory) {
    const link = readLinkedManifest(record);
    const start =
      link.state === 'linked' ? link.manifest.dev?.start : undefined;
    if (!start) return null;
    return {
      start,
      build: link.state === 'linked' ? link.manifest.dev?.build : undefined,
    };
  }
...
export async function pushRemote(
  ...
  if (!record?.remote)
    return { status: 404, body: { error: 'not in remote mode' } };
  const dir =
    record.dev?.workingDirectory ??
    record.sourceDirectory ??
    record.workingDirectory!; // matches the command route's cwd
  if (deps.hasUntrackedEnv(dir))
    return {
      status: 400,
      body: {
        error: 'untracked .env would upload; add it to .gitignore first',
      },
    };
  ...
  if (!commands)
    return {
      status: 400,
      body: { error: `no start command for ${name}, cannot push` },
    };

  const { serviceId } = await deps.railway.ensureService(`deck-${name}`, {
  ...
  const { ok, log } = await deps.railway.up(serviceId, {
    cwd: dir,
    token: deps.token,
  });
  if (!ok) {
    addIssue(name, {
      source: 'railway',
      message: log.slice(0, 300),
      at: new Date().toISOString(),
    });
    return {
      status: 502,
      body: { error: 'railway build failed', log: log.slice(0, 2000) },
    };
  }
  ...
  putRecord({
    ...getRecord(name)!,
    remote: {
      ...getRecord(name)!.remote!,
      serviceId,
      lastPush: { ...prov, at: new Date().toISOString() },
    },
  });
  return {
    status: 200,
    body: { ok: true, lastPush: getRecord(name)!.remote!.lastPush },
  };
}
...
export async function enableRemote(
  ...
  const localOnly = remoteRefuseChecks({
    ...
    railway: deps.railwayConf,
    oauth: deps.oauth,
    hasRailwayToken: deps.hasRailwayToken,
    cfCanEditDns: true,
    zoneSslMode: 'full',
  });
  ...
  const refusal = remoteRefuseChecks({
    ...
    oauth: deps.oauth,
    hasRailwayToken: deps.hasRailwayToken,
    cfCanEditDns: await deps.dns.tokenCanEditDns(),
  ...
  putRecord({
    ...
    remote: {
      ...record.remote,
      target: 'railway',
      serviceId: record.remote?.serviceId ?? '',
      customDomain: host,
      status: 'deploying',
    },
  });
  ...
  const dom = await deps.railway.ensureCustomDomain(
    serviceId,
    host,
    record.port
  );
  ...
  putRecord({
    ...
    remote: {
      ...getRecord(name)!.remote!,
      status: 'verifying',
      cnameTarget: dom.cnameTarget,
      txtName: dom.txtName,
      nextPollAt: new Date(deps.now() + deps.pollBudgetMs).toISOString(),
    },
  });
...
export async function reconcileRemote(deps: {
  railway: RailwayDriver;
  dns: CfDns;
  now(): number;
}): Promise<void> {
...
async function reconcileRemoteOnce(deps: {
  railway: RailwayDriver;
  dns: CfDns;
  now(): number;
}): Promise<void> {
  for (const r of listRecords()) {
    const rem = r.remote;
    if (!rem || (rem.status !== 'deploying' && rem.status !== 'verifying'))
      continue;
    ...
    if (s.verified && s.proxyDetected && rem.cnameTarget) {
      ...
      putRecord({
        ...
        remote: {
          ...rem,
          status: 'live',
          cutover: rem.cutover ?? 'verified-first',
          url: `https://${rem.customDomain}`,
        },
      });
    } else {
      putRecord({
        ...
        remote: {
          ...rem,
          nextPollAt: new Date(deps.now() + POLL_BACKOFF_MS).toISOString(),
        },
      });
    }
  }
...
export async function disableRemote(
  ...
  const record = getRecord(name);
  if (!record?.remote)
    return { status: 200, body: { ok: true, alreadyOff: true } };
