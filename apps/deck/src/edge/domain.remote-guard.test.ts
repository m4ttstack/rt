...
test('re-binding to a different domain refuses while an app is remote-on', async () => {
  ...
  const r = await bindDomain(
    'other.dev',
    { tunnel: fakeTunnel, manager: fakeManager, dns },
    {
      cloudflaredDir: cfDir,
      resolveBin: () => '/opt/homebrew/bin/cloudflared',
      sleep: async () => {},
    }
  );
...
test('--force on a different-domain rebind runs unbind-then-bind and leaves no stranded old wildcard', async () => {
  ...
  const refused = await bindDomain(
    'other.dev',
    { tunnel: fakeTunnel, manager: fakeManager, dns },
    {
      cloudflaredDir: cfDir,
