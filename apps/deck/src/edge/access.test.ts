...
test('oauthToInclude maps each on-mode to CF include rules', () => {
  expect(oauthToInclude({ mode: 'emails', emails: ['m@x.dev'] })).toEqual([
    { email: { email: 'm@x.dev' } },
  ]);
  expect(
    oauthToInclude({ mode: 'emails', emails: ['a@x.dev', 'b@x.dev'] })
  ).toEqual([{ email: { email: 'a@x.dev' } }, { email: { email: 'b@x.dev' } }]);
  expect(oauthToInclude({ mode: 'domains', domains: ['corp.com'] })).toEqual([
    { email_domain: { domain: 'corp.com' } },
  ]);
  expect(
    oauthToInclude({ mode: 'domains', domains: ['corp.com', 'other.dev'] })
  ).toEqual([
    { email_domain: { domain: 'corp.com' } },
    { email_domain: { domain: 'other.dev' } },
  ]);
...
test('sync on a fresh hostname creates the Access app then its allow policy', async () => {
  ...
  await cf.sync('myapp', 'myapp.example.dev', {
    mode: 'emails',
    emails: ['m@x.dev'],
  });
  expect(calls[0]!.url).toContain('/zones/z1/access/apps');
  const create = calls.find(
    c => c.method === 'POST' && c.url.endsWith('/access/apps')
  )!;
  expect(create.body).toMatchObject({
    domain: 'myapp.example.dev',
    type: 'self_hosted',
  });
  const policy = calls.find(c =>
    c.url.includes('/access/apps/app-1/policies')
  )!;
  expect(policy.body).toMatchObject({
    decision: 'allow',
    include: [{ email: { email: 'm@x.dev' } }],
  });
});

// The rt daemon answered ok:true with no creds set yet -- the legitimate
// not-configured state, distinct from unreachable/refused.
const noCredsDeps = {
  deckSecrets: {
    readApiToken: () => 'tok',
...
// The rt daemon answered with the two CF creds, scoped to deck.
function credsDeps(cfApiToken: string, cfZoneId: string) {
  return {
    deckSecrets: {
      readApiToken: () => 'tok',
      post: async () => ({ ok: true, data: { cfApiToken, cfZoneId } }),
    },
  };
}
...
test('syncOAuth without a token degrades loudly, not silently', async () => {
  ...
  const r = await syncOAuth(
    ...
    noCredsDeps as unknown as never
  );
  expect(r.ok).toBe(false);
  expect(getRecord('app-x')!.issues!.some(i => i.source === 'cloudflare')).toBe(
    true
  );
});

test('syncOAuth surfaces a directed rt-daemon-down error, not a silent skip, when the daemon is unreachable', async () => {
  const { putRecord, getRecord, reloadRegistry } =
    await import('../registry/records.ts');
  reloadRegistry();
  putRecord({
  ...
  const r = await syncOAuth(
    'app-down',
    { mode: 'emails', emails: ['m@x.dev'] },
    deps as unknown as never
  );
  expect(r.ok).toBe(false);
  expect(r.message).toContain('rt daemon start');
  const issue = getRecord('app-down')!.issues!.find(
    i => i.source === 'cloudflare'
  );
  expect(issue?.message).toContain('rt daemon start');
});

test("syncOAuth surfaces the daemon's own gate refusal, distinct from unreachable", async () => {
  const { putRecord, getRecord, reloadRegistry } =
    await import('../registry/records.ts');
  reloadRegistry();
  putRecord({
  ...
  expect(r.message).not.toContain('rt daemon start');
});

test('syncOAuth on a CF API error degrades loudly and never leaks the token', async () => {
  const { putRecord, getRecord, reloadRegistry } =
    await import('../registry/records.ts');
  const { updatePlatformSettings, reloadPlatformSettings } =
    await import('../api/platform-settings.ts');
  reloadRegistry();
  reloadPlatformSettings();
  const secretToken = 'secret-token-should-never-appear-12345';
  updatePlatformSettings({ publicDomain: 'example.dev' });
  putRecord({
    name: 'app-err',
    managedBy: 'user',
    port: 11001,
  ...
  const impl = (async (url: string | URL | Request, init?: RequestInit) => {
    ...
    if (method === 'POST' && u.endsWith('/access/apps')) {
      ...
      return Response.json(
        { success: false, errors: [{ code: 1000, message: 'bad request' }] },
        { status: 400 }
      );
    }
    return Response.json({ success: true, result: { id: 'pol-1' } });
  ...
  const r = await syncOAuth(
    'app-err',
    { mode: 'emails', emails: ['m@x.dev'] },
    { accessFetch: impl, ...credsDeps(secretToken, 'z1') } as unknown as never
  );

  expect(r.ok).toBe(false);
  const issue = getRecord('app-err')!.issues!.find(
    i => i.source === 'cloudflare'
  );
...
test('syncOAuth clears its own cloudflare issue once credentials are fixed and a later sync succeeds', async () => {
  ...
  expect(
    getRecord('app-fixed')!.issues!.some(i => i.source === 'cloudflare')
  ).toBe(true);

  // Credentials get fixed, and a later sync against a canned-success fetch
  // must clear the stale badge, not leave it permanent.
  const { impl } = cannedCf();
  const ok = await syncOAuth(
    'app-fixed',
    { mode: 'emails', emails: ['m@x.dev'] },
    { accessFetch: impl, ...credsDeps('tok', 'z1') } as unknown as never
  );
...
test('syncOAuth guards against a null publicDomain instead of building a bogus hostname', async () => {
  ...
  reloadRegistry();
  reloadPlatformSettings();
  updatePlatformSettings({ publicDomain: null });
  putRecord({
    name: 'app-nodomain',
    managedBy: 'user',
    port: 11004,
...
test('syncOAuth off tears the Access app down at a configured zone', async () => {
  ...
  putRecord({
    ...
    createdAt: '2026-08-10T00:00:00Z',
    issues: [
      { source: 'cloudflare', message: 'stale', at: '2026-08-10T00:00:00Z' },
    ],
  });
  ...
  const impl = (async (url: string | URL | Request, init?: RequestInit) => {
    ...
    if (method === 'GET' && u.endsWith('/access/apps')) {
      return Response.json({
        success: true,
        result: [{ id: 'app-7', domain: 'app-off.example.dev' }],
      });
    }
    return Response.json({ success: true, result: null });
  }) as typeof fetch;

  const r = await syncOAuth('app-off', { mode: 'off' }, {
    accessFetch: impl,
    ...credsDeps('tok', 'z1'),
  } as unknown as never);
  ...
  expect(
    calls.some(
      c => c.method === 'DELETE' && c.url.endsWith('/access/apps/app-7')
    )
  ).toBe(true);
  expect(
    getRecord('app-off')!.issues?.some(i => i.source === 'cloudflare')
  ).toBeFalsy();
});
...
test('syncOAuth off never claims success when it could not even attempt the teardown', async () => {
  ...
  const impl = (async () => {
    throw new Error(
      'must not call Cloudflare with no credentials or no domain'
    );
  }) as unknown as typeof fetch;
  ...
  updatePlatformSettings({ publicDomain: 'example.dev' });
  const noCredsResult = await syncOAuth('app-off-unconf', { mode: 'off' }, {
    accessFetch: impl,
    ...noCredsDeps,
  } as unknown as never);
  expect(noCredsResult.ok).toBe(false);
  expect(noCredsResult.message).toContain(
    'Cloudflare API token/zone not configured'
  );
  expect(
    getRecord('app-off-unconf')!.issues!.some(i => i.source === 'cloudflare')
  ).toBe(true);

  // Credentials but no bound domain: still no hostname to remove against.
  updatePlatformSettings({ publicDomain: null });
  const noDomain = await syncOAuth('app-off-unconf', { mode: 'off' }, {
    accessFetch: impl,
    ...credsDeps('tok', 'z1'),
  } as unknown as never);
...
test('CfAccess.sync on an existing hostname updates the app and replaces its policy', async () => {
  ...
  const impl = (async (url: string | URL | Request, init?: RequestInit) => {
    ...
    if (method === 'GET' && u.endsWith('/access/apps')) {
      return Response.json({
        success: true,
        result: [{ id: 'app-9', domain: 'existing.example.dev' }],
      });
    }
    ...
    if (method === 'GET' && u.endsWith('/access/apps/app-9/policies')) {
      return Response.json({
        success: true,
        result: [{ id: 'pol-9', name: 'local tier' }],
      });
    }
  ...
  const cf = new CfAccess({ token: 'tok', zoneId: 'z1', fetchImpl: impl });
  await cf.sync('myapp', 'existing.example.dev', {
    mode: 'emails',
    emails: ['m@x.dev'],
  });
  ...
  const updateApp = calls.find(
    c => c.method === 'PUT' && c.url.endsWith('/access/apps/app-9')
  )!;
  expect(updateApp.body).toMatchObject({
    domain: 'existing.example.dev',
    type: 'self_hosted',
  });
  const replacePolicy = calls.find(
    c =>
      c.method === 'PUT' && c.url.endsWith('/access/apps/app-9/policies/pol-9')
  )!;
  expect(replacePolicy.body).toMatchObject({
    decision: 'allow',
    include: [{ email: { email: 'm@x.dev' } }],
  });
});
