...
export async function reconcileEdge(deps: EdgeReconcileDeps): Promise<void> {
  ...
  try {
    ...
    const expected = expectedTunnelConfig({
      uuid: tunnel.uuid,
      domain: publicDomain,
      cfDir: deps.cloudflaredDir,
    ...
    if (
      ...
      writeTunnelConfig(configPath, {
        uuid: tunnel.uuid,
        credentialsFile: credentialsPath(deps.cloudflaredDir, tunnel.uuid),
        domain: publicDomain,
        gatewayPort: deps.gatewayPort,
    ...
    if (!(await deps.manager.isInstalled(TUNNEL_LABEL))) {
      await deps.manager.install(
        tunnelServiceSpec({ configPath, cloudflaredBin: deps.cloudflaredBin })
      );
      restart = true;
    }
    const svc = (await deps.services()).find(s => s.label === TUNNEL_LABEL);
    if (restart || !svc || svc.pid === null)
    ...
    if (now >= nextCfAt) {
      ...
      if (dns) {
        ...
        if (!drift.tunnelGone) {
          ...
          if (!cur || cur.target !== target || !cur.proxied)
            await dns.writeProxiedCname(host, target);
        }
