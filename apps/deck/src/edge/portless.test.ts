...
const { PortlessCli, FakeEdgeProxy, readProxyTlds } =
  await import('./portless.ts');
...
test('a nonzero exit becomes a thrown error naming the command', async () => {
  const cli = withPath(
    emptyPathDir,
    () => new PortlessCli(async () => ({ code: 1, output: 'boom' }))
  );
  await expect(cli.alias('myapp', 11007)).rejects.toThrow(
    /portless alias myapp/
  );
});
...
test('removeAlias is idempotent: portless reporting No alias found is success, not a failure', async () => {
  const cli = withPath(
    emptyPathDir,
    () =>
      new PortlessCli(async () => ({
        code: 1,
        output: 'No alias found for myapp\n',
      }))
  );
...
test('removeAlias still throws on a genuine portless failure (not the already-gone message)', async () => {
  const cli = withPath(
    ...
    () =>
      new PortlessCli(async () => ({
        code: 1,
        output: 'permission denied writing routes.json\n',
      }))
  );
  await expect(cli.removeAlias('myapp')).rejects.toThrow(
    /portless alias --remove myapp/
  );
});
