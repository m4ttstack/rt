...
const { bindDomain, tunnelConfigPath, TUNNEL_LABEL } =
...
test('binds: mints + creates the tunnel, records identity, config, upserts DNS, installs + kickstarts, sets publicDomain', async () => {
  ...
  expect(tunnel.calls[0]).toEqual([
    'create',
    expect.stringMatching(/^deck-edge-[a-z0-9-]+-abc123$/),
  ]);
  ...
  expect(readFileSync(tunnelConfigPath(), 'utf8')).toContain(
    `tunnel: fake-uuid-1`
  );
  expect(readFileSync(tunnelConfigPath(), 'utf8')).toContain(
    `metrics: 127.0.0.1:7951`
  );
  expect(dns.cname.get('*.example.dev')).toEqual({
    target: 'fake-uuid-1.cfargotunnel.com',
    proxied: true,
  });
...
test('bind polls info() until a connector registers, bounded', async () => {
  ...
  const r = await bindDomain(
    ...
    { tunnel, manager, dns },
    {
      ...
      sleep: async ms => {
        slept.push(ms);
        if (slept.length === 2) tunnel.connectors = 2;
      },
    }
  );
  expect(r.status).toBe(200);
