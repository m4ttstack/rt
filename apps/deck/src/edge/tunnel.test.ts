...
test('create reads the uuid from stdout or stderr', async () => {
  const { driver } = cli({
    'tunnel create': {
      stderr: 'Created tunnel n with id 0f2f1c9e-1b2c-4d3e-8f9a-0b1c2d3e4f5a',
    },
  });
  expect(await driver.create('n')).toEqual({
    uuid: '0f2f1c9e-1b2c-4d3e-8f9a-0b1c2d3e4f5a',
  });
});
...
test('writeTunnelConfig creates parent dirs and writes the rendered file', () => {
  ...
  writeTunnelConfig(path, {
    uuid: 'u',
    credentialsFile: '/c/u.json',
    domain: 'e.dev',
    gatewayPort: 1,
    metricsPort: 2,
  });
