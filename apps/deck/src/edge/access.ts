...
export class CfAccess {
  ...
  private async findAppByHostname(
    hostname: string
  ): Promise<CfAccessApp | undefined> {
  ...
  async sync(appName: string, hostname: string, rule: OAuth): Promise<void> {
    ...
    if (existing) {
      app = await this.req<CfAccessApp>(
        'PUT',
        `/access/apps/${existing.id}`,
        payload
      );
      ...
      policies = await this.req<CfAccessPolicy[]>(
        'GET',
        `/access/apps/${app.id}/policies`
      );
    } else {
    ...
    if (existingPolicy) {
      await this.req(
        'PUT',
        `/access/apps/${app.id}/policies/${existingPolicy.id}`,
        policyPayload
      );
    } else {
...
export async function syncOAuth(
  ...
  if (rule.mode === 'off') {
    ...
    if (!cfApiToken || !cfZoneId) {
      const message =
        'Cloudflare API token/zone not configured, any Access app at the edge was left in place.';
      addIssue(app, {
        source: 'cloudflare',
        message,
        at: new Date().toISOString(),
    ...
    if (!settings.publicDomain) {
      const message =
        'No domain bound, so no hostname could be torn down at Cloudflare.';
      addIssue(app, {
        source: 'cloudflare',
        message,
        at: new Date().toISOString(),
    ...
    try {
      const cf = new CfAccess({
        token: cfApiToken,
        zoneId: cfZoneId,
        fetchImpl: deps.accessFetch,
      });
  ...
  if (!settings.publicDomain) {
    ...
    const message =
      'No domain bound yet, bind a domain before requiring sign-in.';
    addIssue(app, {
      source: 'cloudflare',
      message,
      at: new Date().toISOString(),
