...
import { FakeCfDns, FakeRailwayDriver } from '../../test/fixture/remote.ts';
import {
  getRecord,
  putRecord,
  reloadRegistry,
...
import { disableRemote, enableRemote } from './remote.ts';
...
beforeEach(() => {
  process.env.LOCAL_REGISTRY_PATH = `/tmp/deck-${crypto.randomUUID()}.json`;
  reloadRegistry();
  putRecord(rec);
});
...
function deps(rw: FakeRailwayDriver, dns: FakeCfDns, over: Partial<any> = {}) {
  return {
    ...
    oauth: { mode: 'emails' as const },
    railwayConf: { projectId: 'p', environmentId: 'e' },
    hasRailwayToken: true,
    provenance: () => ({ sha: 's', dirty: false }),
    hasUntrackedEnv: () => false,
    pollBudgetMs: 600000,
    sleep: async () => {},
    now: (() => {
      let t = 0;
...
test('verified-first: TXT is written before the CNAME, CNAME is last, status live', async () => {
  ...
  expect(getRecord('site')!.remote!.txtName).toBe(
    '_railway-verify.site.m4tthew.dev'
  );
});
...
test('cname-first fallback: never verifies within budget, writes CNAME anyway', async () => {
  const rw = new FakeRailwayDriver();
  const dns = new FakeCfDns(); // never setVerified
  const r = await enableRemote('site', deps(rw, dns));
  expect(r.status).toBe(200);
...
test('refuse-check failure aborts before any driver call', async () => {
  const rw = new FakeRailwayDriver();
  const dns = new FakeCfDns();
  const r = await enableRemote(
    'site',
    deps(rw, dns, { oauth: { mode: 'off' } })
  );
