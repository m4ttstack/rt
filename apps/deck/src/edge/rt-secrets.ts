...
export type DeckSecretsResult =
  ({ ok: true } & DeckCfSecrets) | { ok: false; message: string };
...
export interface RtSecretsDeps {
  ...
  post?: (payload: {
    token: string;
    scope: 'deck';
  }) => Promise<RtResponse<RawDeckSecretsData>>;
}
...
function defaultPost(payload: {
  ...
  return rtCommand<RawDeckSecretsData>('secrets:read', payload, {
    sockPath: sockPath(),
    timeoutMs: 15_000,
  });
}
...
const UPDATE_RT_MESSAGE =
  'the rt daemon predates the deck scope — update rt and restart the daemon';
...
export async function readDeckSecrets(
  deps: RtSecretsDeps = {}
): Promise<DeckSecretsResult> {
  ...
  const data = res.data ?? {};
  const hasCfKeys =
    data.cfApiToken !== undefined || data.cfZoneId !== undefined;
  const hasExtensionKeys =
    data.linearApiKey !== undefined || data.gitlabToken !== undefined;
  ...
  return {
    ...
    cfDnsToken: data.cfDnsToken,
    railwayToken: data.railwayToken,
    railwayApiToken: data.railwayApiToken,
  };
}
