...
): Promise<{ up: boolean; readyConnections: number }> {
  try {
    const res = await fetchImpl(`http://127.0.0.1:${port}/ready`, {
  ...
  switch (state) {
    case 'connected':
      return {
        ...
        ok: true,
        tone: 'ok',
        detail: `${readyConnections} connection${readyConnections === 1 ? '' : 's'}`,
      };
    case 'disconnected':
      return {
        ...base,
    ...
    case 'gone':
      return {
        ...
        hint: `re-run deck domain ${domain ?? '<domain>'}`,
      };
  }
}
...
async function probeEdge(
  d: EdgeProbe
): Promise<{ state: EdgeState; readyConnections: number }> {
  const ready = d.running
    ? await readReady(d.fetchImpl)
    : { up: false, readyConnections: 0 };
  const state = edgeState({
    running: d.running && ready.up,
    readyConnections: ready.readyConnections,
    tunnelGone: d.tunnelGone,
  });
...
export async function describeEdge(d: EdgeProbe & { installed: boolean }) {
  if (!d.installed)
    return {
      ...
      readyConnections: 0,
      detail: 'tunnel service not installed',
    };
  const { state, readyConnections } = await probeEdge(d);
