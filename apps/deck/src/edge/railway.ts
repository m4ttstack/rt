export interface RailwayDriver {
  ...
  domainStatus(
    serviceId: string,
    host: string
  ): Promise<{ verified: boolean; proxyDetected: boolean }>;
...
interface DomainJson {
  ...
  domain?: {
    ...
    verification?: { verified: boolean; dnsHost: string; token: string };
  };
}
interface DnsRecord {
  recordType: string;
...
export class RailwayCli implements RailwayDriver {
  ...
  private get exec(): ExecOut {
    return this.cfg.exec ?? realExec;
  }
  private get doFetch(): FetchLike {
    return this.cfg.doFetch ?? fetch;
  }
  ...
  private async graphql<T>(
    ...
    if (!res.ok)
      throw new Error(`railway graphql ${res.status}: ${text.slice(0, 300)}`);
    ...
    if (json.errors?.length)
      throw new Error(
        `railway graphql error: ${json.errors
          .map(e => e.message)
          .join('; ')
          .slice(0, 300)}`
      );
  ...
  async configureService(
    ...
    await this.graphql(
      ...
      {
        s: serviceId,
        e: this.cfg.environmentId,
        i: {
          startCommand: cfg.startCommand,
          ...(cfg.buildCommand ? { buildCommand: cfg.buildCommand } : {}),
        },
      }
    );
    await this.graphql(
      `mutation($i:VariableCollectionUpsertInput!){ variableCollectionUpsert(input:$i) }`,
      {
        i: {
          ...
          serviceId,
          variables: { ...cfg.variables, PORT: String(cfg.port) },
          skipDeploys: true,
        },
      }
    );
  ...
  async ensureCustomDomain(
    ...
    if (add.code === 0) {
      ...
      throw new Error(
        `railway domain ${host} failed: ${add.stdout.slice(0, 300)}`
      );
    } else {
      created = false;
    }
    // Status carries the TXT ownership record (and the CNAME when we resumed).
    const st = await this.railway([
      'domain',
      'status',
      host,
      '--service',
    ...
    if (st.code !== 0)
      throw new Error(
        `railway domain status ${host} failed: ${st.stdout.slice(0, 300)}`
      );
    ...
    if (!v)
      throw new Error(`railway domain status ${host}: no verification record`);
  ...
  async domainStatus(
    ...
    const verified =
      (JSON.parse(st.stdout) as DomainJson).domain?.verification?.verified ??
      false;
  ...
  async removeCustomDomain(serviceId: string, host: string): Promise<void> {
    const { code, stdout } = await this.railway([
      'domain',
      'delete',
      host,
      '-y',
    ...
    if (code !== 0 && !/not found|no such|does not/i.test(stdout)) {
      throw new Error(
        `railway domain delete ${host} failed: ${stdout.slice(0, 300)}`
      );
    }
  }

  async deleteService(serviceId: string): Promise<void> {
    await this.graphql(`mutation($id:String!){ serviceDelete(id:$id) }`, {
      id: serviceId,
    });
  }
}
