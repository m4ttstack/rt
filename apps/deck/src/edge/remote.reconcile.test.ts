...
test('not-yet-verified sets a future nextPollAt and does not busy-loop', async () => {
  ...
  expect(Date.parse(getRecord('site')!.remote!.nextPollAt!)).toBeGreaterThan(
    1000
  );
});
...
test('a live record is skipped (no poll)', async () => {
  ...
  const rw = new FakeRailwayDriver();
  const spy: string[] = [];
  const orig = rw.domainStatus.bind(rw);
  rw.domainStatus = async (...a) => {
    spy.push('polled');
    return orig(...a);
  };
  ...
  putRecord(
    verifyingRec({
      remote: {
        ...
        status: 'verifying',
        cnameTarget: 'kw1ig666.up.railway.app',
        nextPollAt: new Date(2000).toISOString(),
      },
    })
  );
  const rw = new FakeRailwayDriver();
