...
const { bindDomain, unbindDomain, TUNNEL_LABEL, tunnelConfigPath } =
...
test('unbind happy path: uninstall, delete DNS, forced tunnel delete + creds, config, state', async () => {
  ...
  const r = await unbindDomain(
    { tunnel, manager, dns },
    { cloudflaredDir: cfDir, force: true }
  );
  expect(r.status).toBe(200);
  ...
  expect(getPlatformSettings()).toMatchObject({
    publicDomain: null,
    tunnel: null,
  });
});
...
test('unbind lists tunnel-served apps that will go offline and requires force', async () => {
  ...
  expect(r.status).toBe(409);
  expect(r.body as any).toEqual({
    error: 'apps-will-go-offline',
    apps: ['blog'],
  });
  expect(
    (
...
test('unbind tolerates a partial bind (tunnel recorded, no domain): skips DNS, still removes the rest', async () => {
  updatePlatformSettings({
    tunnel: { name: 'deck-edge-mbp-abc123', uuid: 'u-partial' },
  });
