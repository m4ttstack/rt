...
# Auth: set NOTARY_PROFILE (a notarytool keychain-profile name, created once
# via `notarytool store-credentials`) for local runs, or set all three of
# APPLE_ID/APPLE_ID_PASSWORD/APPLE_TEAM_ID for ephemeral CI runs.
set -euo pipefail

if [ "$#" -ne 1 ]; then
    echo "usage: notarize.sh <target.app|target.dmg>" >&2
    echo "  auth: NOTARY_PROFILE, or APPLE_ID + APPLE_ID_PASSWORD + APPLE_TEAM_ID" >&2
    exit 2
fi
TARGET="$1"
...
RESULT="$(xcrun notarytool submit "$SUBMIT" "${AUTH_ARGS[@]}" --wait --timeout 45m --output-format json)" || true

ID="$(printf '%s' "$RESULT" | /usr/bin/plutil -extract id raw -o - - 2>/dev/null || printf '%s' "$RESULT" | sed -n 's/.*"id" *: *"\([^"]*\)".*/\1/p' | head -1)"
STATUS="$(printf '%s' "$RESULT" | /usr/bin/plutil -extract status raw -o - - 2>/dev/null || printf '%s' "$RESULT" | sed -n 's/.*"status" *: *"\([^"]*\)".*/\1/p' | head -1)"
