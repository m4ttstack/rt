...
The Command reference section in the sidebar is generated from the same command tree that drives rt's interactive pickers, so it always matches what `rt` actually does. See [rt status](/reference/status) or [rt cd](/reference/cd) for examples of what a reference page looks like.
