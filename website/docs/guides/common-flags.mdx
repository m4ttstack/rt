...
Every command and subcommand accepts `--help` (or `-h`), at any depth:

```bash
rt --help              # top-level command listing
rt daemon --help       # subcommand listing for daemon
rt worktree provision -h   # usage, description, and flags for one verb
```

On a branch command this prints the subcommand listing with descriptions. On a leaf verb it prints the usage line, description, aliases, and each declared argument and flag with its hint and default. Help goes to stdout and exits `0`, with color only when stdout is a terminal, so it is safe to pipe.

Two boundaries to know:

- `--help` is only recognized as the *first* token after the command path. A later `--help` reaches the command untouched, so it can never be mistaken for a flag's value.
- Plugin commands that wrap an external binary (`exec` plugins) forward `--help` to that binary instead, which owns its own help. Commands that route verbs internally rather than through the command tree (`rt chat` and `rt agent`) answer a verb-level `--help` (for example `rt chat join --help`) with the command's own usage line, which lists its verbs.
...
Pre-select a repo instead of resolving it from the current working directory. Commands whose context is a worktree (they normally resolve "which repo/worktree am I in" from `cwd`, or show a repo picker) accept `--repo <name>` to skip that resolution and jump straight to `<name>`'s worktree picker (or directly into its one worktree, if it only has one). For example, `rt commit --repo <name>` resolves `<name>` and jumps to its worktree picker instead of resolving the repo from `cwd`.
...
Not every command accepts every flag above; each command's own reference page lists the flags it declares, and the row there says what the flag does for that command. `--json` and `--dry-run` appear on many commands. The conflict behavior and the `--agent` / `--no-agent` pair belong to `rt sync` and `rt git rebase` (including `rt git rebase onto`), the commands that run rebases and can hit conflicts; `rt gate open --agent` is unrelated and takes an agent id. The dispatcher's `--repo <name>` works on every worktree-context command, such as `rt commit`, without appearing on its reference page. A `--repo` that a reference page does list (`rt cd`, the `rt worktree` verbs, `rt settings`, `rt runs`, and others) is that command's own flag, with the meaning its row gives.
