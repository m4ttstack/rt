...
- Plugin commands that wrap an external binary (`exec` plugins) forward `--help` to that binary instead, which owns its own help. Commands that route verbs internally rather than through the command tree (for example `rt chat join`) answer a verb-level `--help` with that verb's own usage line.
