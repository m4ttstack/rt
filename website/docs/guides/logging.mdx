...
- **Menu bar app.** mattstack.app's own logging API is the only way it logs (never a bare system log call), and it logs every non-2xx reply it sends.
...
Every surface (daemon, CLI, the menu bar app's `tray` surface, and any future surface that follows the same convention) appends JSON lines to:

```
~/.mattstack/rt/logs/<surface>.YYYY-MM-DD[.N].log
```

For example, `cli.2026-07-17.log`, `tray.2026-07-17.log`, or `daemon.2026-07-17.1.log` (the daemon's rotating writer always numbers its files, and starts a new one when a file passes 50 MB).

`rt daemon logs` auto-discovers surfaces by matching this filename pattern in `~/.mattstack/rt/logs/` and following the newest file per surface. That means a new surface which adopts the convention shows up in the log viewer automatically, with nothing to register.
