...
When the team declares a switchboard, `rt team join` confirms its URL for the joiner, so the `account.switchboard` row reads ready and Install is reachable without a terminal.
