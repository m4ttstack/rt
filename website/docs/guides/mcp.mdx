...
Unlike the other tools, `rt_verb` does not talk to the daemon; it runs rt itself as a child process. Only verbs marked agent-safe are allowed to run through it, curated to a short list of read-only commands (today: `worktree list`, `endpoint lookup`, `herd status`). Anything else is refused, and the refusal message lists the verbs that do run. Pass `args` without the leading `rt` (e.g. `["worktree", "list"]`), and pass `cwd` when the verb depends on the current repo, since the server's own working directory is fixed at session start and does not follow `cd` or `EnterWorktree`.

## When the daemon is down

Every daemon-backed tool connects over its Unix socket. When the daemon is not running, those tools return an error with a human-readable explanation. Chat tools also fail early if no signed-in session exists. `rt_verb` does not depend on the daemon at all.
