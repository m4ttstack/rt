...
Unlike the other tools, `rt_verb` does not itself connect to the daemon; it runs rt as a child process instead of calling the daemon's Unix socket directly. The verb it runs may still talk to the daemon internally (today's three do: `worktree list`, `endpoint lookup` and `herd status` are all daemon-backed reads), so `rt_verb` is not a way to reach these commands while the daemon is down, only a way to run them without a direct socket connection from the MCP server process. Only verbs marked agent-safe are allowed to run through it. Anything else is refused, and the refusal message lists the verbs that do run. Pass `args` without the leading `rt` (e.g. `["worktree", "list"]`), and pass `cwd` when the verb depends on the current repo, since the server's own working directory is fixed at session start and does not follow `cd` or `EnterWorktree`.
