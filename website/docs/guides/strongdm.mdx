...
rt sdm connections --json  # list connection keys, for scripts and agents
...
See the full command reference: [`rt sdm`](/reference/sdm), [`rt sdm connect`](/reference/sdm/connect), [`rt sdm connections`](/reference/sdm/connections), [`rt sdm status`](/reference/sdm/status), [`rt sdm login`](/reference/sdm/login), [`rt sdm refresh`](/reference/sdm/refresh), [`rt sdm enrichment`](/reference/sdm/enrichment), [`rt sdm set-email`](/reference/sdm/set-email).

## rt reads live, it does not maintain a list

rt reads your resources straight from the StrongDM CLI: `sdm access catalog` for the datasources you can reach, and `sdm status` for what is currently connected. There is nothing to configure and no list to maintain in rt itself. Every Postgres-family datasource you can reach shows up in the picker (connect and verify are Postgres-oriented, so other resource types are left out), alongside your recent connections; rt never invents or guesses a resource name. The scan is cached for 10 minutes; `rt sdm refresh` re-scans now.
...
Raw StrongDM names (`example-alpha-staging`) work as-is, but you can give them nicer labels, group them by tier, and set connect defaults with a declarative file you own at `~/.mattstack/rt/sdm/enrichment.jsonc`. Nothing runs to enrich; rt just reads this JSON and overlays it on the live catalog.

Scaffold it from your current catalog, then fill in the labels:

```bash
rt sdm enrichment init    # writes ~/.mattstack/rt/sdm/enrichment.jsonc: every resource, blank labels
...
A resource missing from the file just shows its raw name and connects with Postgres defaults.

To share enrichment with your team, put the same map in the team settings store under `rt.sdmEnrichment` (`rt settings set rt.sdmEnrichment '<json>' --scope team`). Once the team store holds that key it replaces the local file wholesale, and `rt sdm enrichment init` declines to write one.
