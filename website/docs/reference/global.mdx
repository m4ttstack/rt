...
- **`--daemon`** starts the daemon server process directly. It's the entry point the daemon supervisor spawns internally (`rt --daemon`) when `rt` is a compiled binary; you shouldn't need to run it yourself.
