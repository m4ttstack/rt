...
export function metricValue(m: UserMetrics, d: MetricDescriptor): number | null {
  const c = cell(m, d);
  return d.kind === "scalar" ? (c as MetricValue).value : (c as DistributionValue).p50;
}

/** The trend delta: scalar delta, or distribution deltaP50. */
export function metricDelta(m: UserMetrics, d: MetricDescriptor): number | null {
  const c = cell(m, d);
  return d.kind === "scalar" ? (c as MetricValue).delta : (c as DistributionValue).deltaP50;
}
...
export function setMetricRank(m: UserMetrics, d: MetricDescriptor, rank: number | null): void {
  cell(m, d).rank = rank;
}
