...
function rowOrder(): string[] {
  return screen.getAllByTestId(/^row-/).map((el) => el.dataset.testid!.replace("row-", ""));
}
...
describe("LeaderboardTable", () => {
  ...
  it("marks the current user's row and leaves other rows unmarked", () => {
    ...
    expect(screen.getByTestId("row-bob")).toHaveAttribute("data-current-user", "true");
    expect(screen.getByTestId("row-alice")).not.toHaveAttribute("data-current-user");
  });
  ...
  it("renders a delta badge with the sign of the trend when trend is on", () => {
    ...
    const { unmount } = renderTable(<LeaderboardTable data={improving} trend />);
    const up = screen.getByText(/▲ 4/);
    expect(up).toHaveAttribute("data-good", "true");
    unmount();

    const worsening = buildResponse(
      [buildUser("alice", buildMetrics({ mrsMerged: { value: 6, delta: -3 } }))],
      { hasTrend: true },
    );
