...
function buildDetail(overrides: Partial<UserDetailResponse> = {}): UserDetailResponse {
  return {
    ...
    fromCache: true,
    user: buildUser("alice", buildMetrics({ mrsMerged: { value: 9 }, additions: { value: 100 } })),
    evidence: {
      mrsMerged: { columns: ["MR", "Title"], rows: [{ cells: ["!1", "Fix the thing"], href: "https://gitlab.example.com/mr/1" }] },
...
describe("DetailPage", () => {
  it("renders the route's initial stat's evidence, and switches to another stat's evidence on rail selection", () => {
    useUserDetail.mockReturnValue({ data: buildDetail(), error: null, isLoading: false });

    renderWithProviders(<DetailPage username="alice" initialStat="mrsMerged" range={{ range: "30d" }} trend={false} />);
  ...
  it("falls back to the first column when the route's stat is not a real metric key", () => {
    ...
    renderWithProviders(<DetailPage username="alice" initialStat="not-a-real-metric" range={{ range: "30d" }} trend={false} />);
  ...
  it("shows the error message and not a blank page when the detail query fails", () => {
    useUserDetail.mockReturnValue({ data: undefined, error: new Error("boom"), isLoading: false });
  ...
  it("shows a loading state while the detail query is in flight and there is no data yet", () => {
    useUserDetail.mockReturnValue({ data: undefined, error: null, isLoading: true });
