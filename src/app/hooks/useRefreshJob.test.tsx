...
describe("useRefreshJob: start() in-flight guard", () => {
  ...
  it("does not start a second job while one is already in flight (regression: window-refocus double start)", async () => {
    ...
    const onDone = vi.fn();
    const onError = vi.fn();
    const { result } = renderHook(() => useRefreshJob({ onDone, onError }), { wrapper });
    ...
    await act(async () => {
      resolvePost({ ok: true, status: 200, json: async () => ({ jobId: "job-1", status: "running", progress: null }) });
    });
  ...
  it("still starts a new job once the previous one has finished (manual refresh keeps working)", async () => {
    postMock
      .mockResolvedValueOnce({ ok: true, status: 200, json: async () => ({ jobId: "job-1", status: "done", progress: null, result: undefined }) })
      .mockResolvedValueOnce({ ok: true, status: 200, json: async () => ({ jobId: "job-2", status: "running", progress: null }) });
