...
function slackApi(method: string, params: Record<string, string>): Response {
  switch (method) {
    ...
    case "conversations.list":
      return ok({
        channels: Object.entries(CHANNEL_IDS).map(([name, id]) => ({ id, name })),
    ...
    case "conversations.history": {
      ...
      return ok({
        messages: mrUrl ? [{ ts: "100.000001", user: "U1", text: `please review ${mrUrl}` }] : [],
        has_more: false,
      });
    }
    case "reactions.get":
...
globalThis.fetch = (async (input: Parameters<typeof fetch>[0], init?: Parameters<typeof fetch>[1]) => {
  ...
  if (!url.startsWith("https://slack.com/api/")) return realFetch(input as never, init);
