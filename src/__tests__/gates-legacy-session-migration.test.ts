...
let dir: string;
beforeEach(() => { dir = mkdtempSync(join(tmpdir(), "lsm-")); });
...
function migrateReview(logged: string[]): void {
  migrateLegacySessions("review", readReviewStates(dir), (url) => reviewFilePath(url, dir), writeReviewState, (m) => logged.push(m));
}

function migrateRespond(logged: string[]): void {
  migrateLegacySessions("respond", readRespondStates(dir), (url) => respondFilePath(url, dir), writeRespondState, (m) => logged.push(m));
}

describe("migrateLegacySessions", () => {
  test("clears sessionId on a legacy review state (sessionId set, no agentId)", () => {
    writeReviewState(reviewFilePath(URL_A, dir), { mrUrl: URL_A, iid: 4821, status: "done", sessionId: "sess-1" });
    const logged: string[] = [];
  ...
  test("clears sessionId on a legacy respond state (sessionId set, no agentId)", () => {
    writeRespondState(respondFilePath(URL_A, dir), { mrUrl: URL_A, iid: 4821, status: "done", sessionId: "sess-1" });
    const logged: string[] = [];
  ...
  test("leaves an agentId-bearing state untouched", () => {
    writeReviewState(reviewFilePath(URL_A, dir), { mrUrl: URL_A, iid: 4821, status: "done", sessionId: "sess-1", agentId: "agent-1" });
    const logged: string[] = [];
    migrateReview(logged);
  ...
  test("no-ops on a clean state with neither sessionId nor agentId", () => {
    writeReviewState(reviewFilePath(URL_A, dir), { mrUrl: URL_A, iid: 4821, status: "queued" });
    const logged: string[] = [];
    migrateReview(logged);
  ...
  test("preserves other fields, and only migrates the legacy row among several", () => {
    writeReviewState(reviewFilePath(URL_A, dir), {
      mrUrl: URL_A, iid: 4821, status: "done", sessionId: "sess-1", tabId: "w1:t1", outcome: "approve",
    });
    writeReviewState(reviewFilePath(URL_B, dir), { mrUrl: URL_B, iid: 4822, status: "done", sessionId: "sess-2", agentId: "agent-2" });
    const logged: string[] = [];
    migrateReview(logged);
