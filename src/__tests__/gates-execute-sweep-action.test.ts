...
import { executeSweepAction, type DomainStateIo, type ExecuteSweepActionIo } from "../gates/execute-sweep-action.ts";
...
function makeIo(parkResult: { ok: true } | { ok: false; error: string }): ExecuteSweepActionIo & {
  ...
  const calls: string[] = [];
  const closed: string[] = [];
  const writes: Record<"review" | "respond" | "doctor", unknown[]> = { review: [], respond: [], doctor: [] };
  const logs: string[] = [];
  const domainIo = (domain: "review" | "respond" | "doctor"): DomainStateIo => ({
    writeState: (_path, patch) => { writes[domain].push(patch); },
...
describe("executeSweepAction", () => {
  test("park action calls gatePark before closeTab, and logs on success", async () => {
    ...
    const action: SweepAction = { kind: "park", domain: "review", mrUrl: MR_URL, tabId: "tab-1", gateId: "gate-1" };
  ...
  test("park action with no tabId calls gatePark but never closeTab", async () => {
    const io = makeIo({ ok: true });
    const action: SweepAction = { kind: "park", domain: "review", mrUrl: MR_URL, gateId: "gate-1" };
    await executeSweepAction(action, io);
  ...
  test("park action with no gateId is a no-op", async () => {
    const io = makeIo({ ok: true });
    const action: SweepAction = { kind: "park", domain: "review", mrUrl: MR_URL, tabId: "tab-1" };
    await executeSweepAction(action, io);
  ...
  test("close-missed-done action always closes the tab and writes done to its own domain, no gate re-read guard", async () => {
    const io = makeIo({ ok: true });
    const action: SweepAction = { kind: "close-missed-done", domain: "review", mrUrl: MR_URL, tabId: "tab-2" };
    await executeSweepAction(action, io);
  ...
  test("a doctor close-missed-done writes only the doctor domain's state, never review's or respond's", async () => {
    const io = makeIo({ ok: true });
    const action: SweepAction = { kind: "close-missed-done", domain: "doctor", mrUrl: MR_URL, tabId: "doctor-tab" };
    await executeSweepAction(action, io);
  ...
  test("a respond close-missed-done writes only the respond domain's state", async () => {
    const io = makeIo({ ok: true });
    const action: SweepAction = { kind: "close-missed-done", domain: "respond", mrUrl: MR_URL, tabId: "respond-tab" };
    await executeSweepAction(action, io);
