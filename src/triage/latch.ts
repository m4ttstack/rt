...
export async function runLatchPass(deps: LatchPassDeps): Promise<LatchPassResult> {
  ...
  for (const mr of mrs) {
    ...
    try {
      ...
      console.error(`latch pass: !${mr.iid} (${mr.mrUrl}) failed: ${err instanceof Error ? err.message : err}`);
      result.failed++;
    }
  }
  return result;
...
async function consume(
  ...
  for (const c of carriers) {
    ...
    await spendLatch(deps.gateway, mr.projectId, mr.projectPath, mr.iid, c, "duplicate");
  }
}
...
async function spendAll(deps: LatchPassDeps, mr: LatchMrFacts, latches: LatchRef[]): Promise<void> {
  await spendAllLatches(deps.gateway, mr.projectId, mr.projectPath, mr.iid, latches);
}
