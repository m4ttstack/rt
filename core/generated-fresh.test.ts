...
import { buildGatewayCss } from "../scripts/build-gateway-css.ts";
...
// @ts-expect-error — tsc has no ambient module declaration for a .css text import (with {type:"text"}); runtime is correct, see core/board-assets.test.ts
import COMMITTED_GATEWAY_CSS from "./generated/gateway.css" with { type: "text" };
...
test("core/generated/gateway.css is fresh — run `bun run build:board` after editing tui-kit tokens", () => {
  expect(buildGatewayCss()).toBe(COMMITTED_GATEWAY_CSS as unknown as string);
});
