...
function snapshot() {
  return computeSnapshot(FETCH, { window: WINDOW, users: USERS, sizeBand: SIZE_BAND });
}
...
describe("computeSnapshot", () => {
  ...
  it("pipelines (4.4): in-window count + status breakdown", () => {
    ...
    expect(alice.pipelineStatus).toEqual({ success: 1, failed: 1, canceled: 0, other: 0 });
...
describe("buildResponse trend deltas (4.11)", () => {
  const ctx = (priorW: ReturnType<typeof priorWindow> | null): BuildContext => ({
  ...
  it("flags the tier-fallback note when approvals are unavailable", () => {
    ...
    const snap = computeSnapshot(noApprovals, { window: WINDOW, users: USERS, sizeBand: SIZE_BAND });
