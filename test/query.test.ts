...
const W_7D: TimeWindow = { start: "2026-07-08T00:00:00.000Z", end: "2026-07-15T00:00:00.000Z", key: "7d" };
const W_7D_PRIOR: TimeWindow = { start: "2026-07-01T00:00:00.000Z", end: "2026-07-08T00:00:00.000Z", key: "7d" };
...
const metrics = (over: Partial<StoredMetrics> & Pick<StoredMetrics, "projectPath" | "iid">): StoredMetrics => ({
  description: "desc",
...
describe("buildFetchResult: window characterization (ported from test/slicing.test.ts)", () => {
  ...
  function seedWideScenario() {
    ...
    s.upsertPipelines([
      { id: "gitlab:pipeline:1", projectPath: "acme/app", username: "alice", status: "success", createdAt: "2026-07-09T01:00:00.000Z" },
      { id: "gitlab:pipeline:2", projectPath: "acme/app", username: "alice", status: "failed", createdAt: "2026-07-10T01:00:00.000Z" },
      { id: "gitlab:pipeline:3", projectPath: "acme/app", username: "alice", status: "success", createdAt: "2026-05-01T01:00:00.000Z" }, // outside W_7D
    ]);
    s.upsertPushEvents([
      { username: "alice", createdAt: "2026-07-09T01:00:00.000Z", repositoryId: "acme/app" },
      { username: "alice", createdAt: "2026-07-07T12:00:00.000Z", repositoryId: "acme/app" }, // inside the -1d pad of W_7D
      { username: "alice", createdAt: "2026-05-01T01:00:00.000Z", repositoryId: "acme/app" }, // outside even the pad
    ]);
  ...
  it("keeps push events inside the window's +/-1d pad, mirroring the fetch", () => {
    ...
    expect(fetched.pushEvents.map((e) => e.createdAt).sort()).toEqual(
      ["2026-07-07T12:00:00.000Z", "2026-07-09T01:00:00.000Z"],
    );
  });
  ...
  it("slices the prior window to a disjoint, non-empty set", () => {
    ...
    expect(prior.mrs.map((m) => m.iid)).not.toEqual(expect.arrayContaining([1, 2, 4, 5]));
  });
  ...
  it("excludes an MR updated after the prior window ends", () => {
    ...
    s.upsertIndexRows([
      row({ iid: 8, createdAt: "2026-07-02T00:00:00.000Z", updatedAt: "2026-07-14T00:00:00.000Z", mergedAt: "2026-07-02T12:00:00.000Z" }),
    ]);
    const prior = buildFetchResult(s, W_7D_PRIOR, ["alice", "bob"]);
  ...
  it("joins an index row with its metrics: notes, diffStats, fileStats, labels, approvers, and derived counts", () => {
    ...
    s.upsertMrMetrics([
      metrics({
        ...
        notes: [{ authorUsername: "bob", createdAt: "2026-07-09T02:00:00.000Z", system: false, inline: true }],
      }),
    ]);
    const fetched = buildFetchResult(s, W_7D, ["alice", "bob"]);
    ...
    expect(m.diffStats).toEqual([{ path: "src/a.ts", additions: 12, deletions: 4 }]);
    expect(m.labels).toEqual(["bug", "ui"]);
    expect(m.approvedByUsernames).toEqual(["bob"]);
    expect(m.notes).toEqual([{ authorUsername: "bob", createdAt: "2026-07-09T02:00:00.000Z", system: false, inline: true }]);
  });
...
describe("buildFetchResult: roster scope", () => {
  it("filters pipelines and push events to usernames in the roster", () => {
    ...
    s.upsertPipelines([
      { id: "gitlab:pipeline:1", projectPath: "acme/app", username: "alice", status: "success", createdAt: "2026-07-09T01:00:00.000Z" },
      { id: "gitlab:pipeline:2", projectPath: "acme/app", username: "carol", status: "success", createdAt: "2026-07-09T01:00:00.000Z" },
    ]);
    s.upsertPushEvents([
      { username: "alice", createdAt: "2026-07-09T01:00:00.000Z", repositoryId: null },
      { username: "carol", createdAt: "2026-07-09T01:00:00.000Z", repositoryId: null },
    ]);
    const fetched = buildFetchResult(s, W_7D, ["alice", "bob"]);
...
describe("storedIdentities", () => {
  it("returns a Record keyed by username, including unresolved entries", () => {
    const s = getStore();
    s.upsertIdentities([
      { username: "alice", name: "Alice", resolved: true, userId: 7, fetchedAt: "2026-07-01T00:00:00.000Z" },
      { username: "bob", name: null, resolved: false, userId: null, fetchedAt: "2026-07-01T00:00:00.000Z" },
    ]);
