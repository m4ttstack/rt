...
const metrics = (projectPath: string, iid: number, over: Partial<StoredMetrics> = {}): StoredMetrics => ({
  projectPath, iid,
  description: "desc",
  ...
  notes: [{ authorUsername: "bob", createdAt: "2026-05-01T00:00:00Z", system: false, inline: true }],
  ...over,
});
...
describe("identities", () => {
  it("round-trips identity rows by username", () => {
    const s = getStore();
    s.upsertIdentities([
      { username: "ada", name: "Ada L", resolved: true, userId: 7, fetchedAt: "2026-05-01T00:00:00Z" },
      { username: "bob", name: null, resolved: false, userId: null, fetchedAt: "2026-05-01T00:00:00Z" },
    ]);
